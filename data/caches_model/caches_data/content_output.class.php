...
class content_output {
				...
				if(is_array($server_list)) {
					foreach($server_list as $_k=>$_v) {
						if($value && is_array($sel_server) && in_array($_k,$sel_server)) {
							...
							if($downloadlink) {
								$a_k = urlencode(sys_auth("i=$this->id&s=$_v[siteurl]&m=1&f=$fileurl&d=$downloadtype&modelid=$this->modelid&catid=$this->catid", 'ENCODE', wy_base::load_config('system','auth_key')));
		...
		if(is_array($file_list)) {
			foreach($file_list as $_k=>$_v) {	
				if($_v[fileurl]){
					...
					if($downloadlink) {
						$a_k = urlencode(sys_auth("i=$this->id&s=&m=1&f=$_v[fileurl]&d=$downloadtype&modelid=$this->modelid&catid=$this->catid", 'ENCODE', wy_base::load_config('system','auth_key')));
	...
	function video($field, $value) {
		...
		$video_store_db = wy_base::load_model('video_store_model');
		...
		if (is_array($videos) && !empty($videos)) {
			...
			if ($pagenumber>0) {
				...
				$category_db = wy_base::load_model('category_model');
				...
				if ($setting['content_ishtml']) {
					...
					$this->html_root = wy_base::load_config('system','html_root');
