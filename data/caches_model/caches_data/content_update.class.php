...
function editor($field, $value) {
	...
	return $value;
		...
		if(!empty($value) && is_array($value)) {
			if($_GET['a']=='add') {
				$position_data_db = wy_base::load_model('position_data_model');
				$textcontent = array();
				foreach($value as $r) {
					if($r!='-1') {
	...
	function keyword ($field, $value) {
		...
		//加载关键字的数据模型
		$keyword_db = wy_base::load_model('keyword_model');
		$keyword_data_db = wy_base::load_model('keyword_data_model');
