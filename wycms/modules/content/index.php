...
class index {
	...
	protected function _category_priv($catid) {
		...
		$this->category_priv_db = wy_base::load_model('category_priv_model');
...
function ajaxList() {
        ...
        if (isset($_POST)) {
            ...
            $tablename = $this->db->table_name = $this->db->db_tablepre . $MODEL[$modelid]['tablename'];
            $pagesize = isset($_POST['num']) && intval($_POST['num']) <= intval($WAP_SETTING['listnum']) ? intval($_POST['num']) : intval($_POST['num']);
            $offset = ($page - 1) * $pagesize;
			
            $list = $this->db->select($where, 'id,catid,title,description,url,thumb,inputtime', $offset . ',' . $pagesize, 'listorder desc,id desc');
            for($i=0;$i<count($list);$i++)
            {
               $list[$i][inputtime]=date('y-m-d',$list[$i][inputtime]);
  ...
  function ajaxListpl() {
        if (isset($_POST)) {
			...
			$this->dbm = wy_base::load_model('comment_data_model');
			$tablename = $this->dbm->table_name='wy_comment_data_1';
			 $where = array('commentid' => $commentid);
			 
            $list = $this->dbm->select($where, 'commentid,content', $offset . ',' . $pagesize, 'id desc');
            exit(json_encode($list));
        } else {
            exit(json_encode(array()));
        }
