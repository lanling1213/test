...
class content extends admin {
	...
	public function __construct() {
		...
		if(isset($_GET['catid']) && $_SESSION['roleid'] != 1 && ROUTE_A !='pass' && strpos(ROUTE_A,'public_')===false) {
			...
			$this->priv_db = wy_base::load_model('category_priv_model');
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			for($i=1;$i<=$workflow_steps;$i++) {
				...
				$newimg = $r ? '<img src="'.IMG_PATH.'icon/new.png" style="padding-bottom:2px" onclick="window.location.href=\'?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&wy_ssud='.$wy_ssud.'\'">' : '';
				$workflow_menu .= '<a href="?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&wy_ssud='.$wy_ssud.'" '.$current.' ><em>'.L('workflow_'.$i).$newimg.'</em></a><span>|</span>';
			}
			if($workflow_menu) {
				$current = isset($_GET['reject']) ? 'class=on' : '';
				$workflow_menu .= '<a href="?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&wy_ssud='.$wy_ssud.'&reject=1" '.$current.' ><em>'.L('reject').'</em></a><span>|</span>';
			}
            //获取模型的thumb字段配置信息
	...
	public function add() {
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($category['type']==0) {
				...
				//单网页
				$this->page_db = wy_base::load_model('page_model');
	...
	public function delete() {
		if(isset($_GET['dosubmit'])) {
			...
			$html_root = wy_base::load_config('system','html_root');
			if($sethtml) $html_root = '';
			
			$setting = string2array($this->categorys[$catid]['setting']);
			$content_ishtml = $setting['content_ishtml']; 
			$this->db->set_model($modelid);
			$this->hits_db = wy_base::load_model('hits_model');
			$this->queue = wy_base::load_model('queue_model');
			...
			//附件初始化
			$attachment = wy_base::load_model('attachment_model');
			$this->content_check_db = wy_base::load_model('content_check_model');
			$this->position_data_db = wy_base::load_model('position_data_model');
			$this->search_db = wy_base::load_model('search_model');
			//判断视频模块是否安装 
			if (module_exists('video') && file_exists(WY_PATH.'model'.DIRECTORY_SEPARATOR.'video_content_model.class.php')) {
				$video_content_db = wy_base::load_model('video_content_model');
				$video_install = 1;
			}
			$this->comment = wy_base::load_app_class('comment', 'comment');
			$search_model = getcache('search_model_'.$this->siteid,'search');
			$typeid = $search_model[$modelid]['typeid'];
			$this->url = wy_base::load_app_class('url', 'content');
	...
	public function pass() {
		...
		if($workflowid) {
			...
			//更改内容状态
				...
				$modelid = $this->categorys[$catid]['modelid'];
				$this->db->set_model($modelid);
				$this->db->search_db = wy_base::load_model('search_model');
				//审核通过，检查投稿奖励或扣除积分
				if ($status==99) {
					$html = wy_base::load_app_class('html', 'content');
					$this->url = wy_base::load_app_class('url', 'content');
					...
					if (isset($_POST['ids']) && !empty($_POST['ids'])) {
						foreach ($_POST['ids'] as $id) {
							...
							if($setting['presentpoint']>0) {
								wy_base::load_app_class('receipts','pay',0);
	...
	public function public_preview() {
		...
		if($rs['paginationtype']==1) {
			...
			$contentpage = wy_base::load_app_class('contentpage');
		...
		function ajax_manage(type) {
			if(type==1) {
				$.get('?m=content&c=content&a=pass&ajax_preview=1&catid=".$catid."&steps=".$steps."&id=".$id."&wy_ssud=".$wy_ssud."');
			} else if(type==2) {
				$.get('?m=content&c=content&a=pass&ajax_preview=1&reject=1&catid=".$catid."&steps=".$steps."&id=".$id."&wy_ssud=".$wy_ssud."');
			} else if(type==3) {
				$.get('?m=content&c=content&a=delete&ajax_preview=1&dosubmit=1&catid=".$catid."&steps=".$steps."&id=".$id."&wy_ssud=".$wy_ssud."');
			}
	...
	public function public_sub_categorys() {
		...
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					case 'block':
						$strs = "<span class='\$icon_type'>\$add_icon<a href='?m=block&c=block_admin&a=public_visualization&menuid=".$_GET['menuid']."&catid=\$catid&type=list&wy_ssud=".$_SESSION['wy_ssud']."' target='right'>\$catname</a> \$vs_show</span>";
					break;

					default:
						$strs = "<span class='\$icon_type'>\$add_icon<a href='?m=content&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid&wy_ssud=".$_SESSION['wy_ssud']."' target='right' onclick='open_list(this)'>\$catname</a></span>";
						break;
				}
						...
						if ($t=='content') {
							...
							foreach ($model as $modelid) {
								...
								if ($r = $db->count()) { //判断模型下是否有数据
									...
									$db->query('TRUNCATE TABLE `wycms_'.$model_arr[$modelid]['tablename'].'`');
									//开始清理模型data表数据
									...
									$db->query('TRUNCATE TABLE `wycms_'.$model_arr[$modelid]['tablename'].'_data`');
									//删除该模型中在hits表的数据
									$hits_db = wy_base::load_model('hits_model');
									$hitsid = 'c-'.$modelid.'-';
									...
									//删除该模型在search中的数据
									$search_db = wy_base::load_model('search_model');
									...
									$position_db = wy_base::load_model('position_data_model');
									...
									//附件初始化
									//$attachment = wy_base::load_model('attachment_model');
									//$comment = wy_base::load_app_class('comment', 'comment');
									//if(module_exists('comment')){
										//$comment_exists = 1;
									//}
									//foreach ($data as $d) {
							...
							$comment_db = wy_base::load_model('comment_data_model');
							for($i=1;;$i++) {
								...
								if ($comment_db->table_exists(str_replace($comment_db->db_tablepre, '', $comment_db->table_name))) {
									if ($r = $comment_db->count()) {
										...
										$comment_db->query('TRUNCATE TABLE `wycms_comment_data_'.$i.'`');
									}
								} else {
									break;
								}
							...
							$db = wy_base::load_model($t.'_model');
							if ($r = $db->count()) {
								$result = $db->select();
								...
								$db->query('TRUNCATE TABLE `wycms_'.$t.'`');
							}
						}
					}
				}
