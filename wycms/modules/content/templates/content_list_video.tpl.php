...
<!--
	...
	parent.document.getElementById('center_frame').src = '?m=content&c=content&a=public_categorys&type=add&menuid=<?php echo $_GET['menuid'];?>&wy_ssud=<?php echo $_SESSION['wy_ssud'];?>';
...
<a class="add fb" href="javascript:;" onclick=javascript:openwinx('?m=content&c=content&a=add&menuid=&catid=<?php echo $catid;?>&wy_ssud=<?php echo $_SESSION['wy_ssud'];?>','')><em><?php echo L('add_content');?></em></a>　
<a href="?m=content&c=content&a=init&catid=<?php echo $catid;?>&wy_ssud=<?php echo $wy_ssud;?>" <?php if($steps==0 && !isset($_GET['reject'])) echo 'class=on';?>><em><?php echo L('check_passed');?></em></a><span>|</span>
...
<span>|</span><a href="?m=content&c=create_html&a=category&pagesize=30&dosubmit=1&modelid=0&catids[0]=<?php echo $catid;?>&wy_ssud=<?php echo $wy_ssud;?>&referer=<?php echo urlencode($_SERVER['QUERY_STRING']);?>"><em><?php echo L('update_htmls',array('catname'=>$category['catname']));?></em></a>
<?php }?>
</div>
...
<tbody>
	...
	?>
		...
		<?php
		if($r['sysadd']==0) {
			echo "<a href='?m=member&c=member&a=memberinfo&username=".urlencode($r['username'])."&wy_ssud=".$_SESSION['wy_ssud']."' >".$r['username']."</a>"; 
...
function update_param(fid, obj) {
	...
	$.get('index.php', {m:'content', c:'content', a:'update_param', field:field, id:id, value:value, modelid:modelid, wy_ssud:'<?php echo $_SESSION['wy_ssud'];?>'}, function (data){
		if (data!=200){
            $(obj).val(old_data);
		}
	})
