...
class html {
	...
	public function category($catid, $page = 0) {
		...
		if($create_to_html_root) {
			if($this->siteid==1) {
				$file = WYCMS_PATH.$base_file;
			} else {
				$file = WYCMS_PATH.substr($this->html_root,1).$base_file;
			}
			//添加到发布点队列
	...
	*/
	private function createhtml($file, $copyjs = '') {
		...
		if ($copyjs && !file_exists($dir.'/js.html')) {
			@copy(WY_PATH.'modules/content/templates/js.html', $dir.'/js.html');
		}
		$strlen = file_put_contents($file, $data);
		@chmod($file,0777);
		if(!is_writable($file)) {
			$file = str_replace(WYCMS_PATH,'',$file);
