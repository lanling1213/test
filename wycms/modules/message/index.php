...
wy_base::load_app_class('foreground','member');//加载foreground 应用类. 自动判断是否登陆.
...
class index extends foreground {
	function __construct() {
		parent::__construct();
		$this->message_db = wy_base::load_model('message_model');
		$this->message_group_db = wy_base::load_model('message_group_model');
		$this->message_data_db = wy_base::load_model('message_data_model');
	...
	public function public_name() {
		$username = isset($_GET['username']) && trim($_GET['username']) ? (wy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['username'])) : trim($_GET['username'])) : exit('0');
		$member_interface = wy_base::load_app_class('member_interface', 'member');
		if ($username) {
