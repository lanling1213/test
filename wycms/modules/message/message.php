...
class message extends admin {
	function __construct() {
		parent::__construct();
		$this->db = wy_base::load_model('message_model');
		$this->group_db = wy_base::load_model('message_group_model');
	 ...
	 */
	 public function public_name() {
		$tousername = isset($_GET['tousername']) && trim($_GET['tousername']) ? (wy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['tousername'])) : trim($_GET['tousername'])) : exit('0');
	 	//不能发给自己
	...
	public function message_send() {
		if(isset($_POST['dosubmit'])) {
			...
			//LOAD 会员组模型
			$member_group = wy_base::load_model('member_group_model');
			$member_group_infos = $member_group->select('','*','',$order = 'groupid ASC');
			//LOAD 管理员角色模型
			$role = wy_base::load_model('admin_role_model');
