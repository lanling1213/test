...
wy_base::load_sys_class('form', 0, 0);
wy_base::load_app_func('global', 'video');
...
class video extends admin {
	...
	public function __construct() {
		parent::__construct();
		$this->db = wy_base::load_model('video_store_model');
		$this->module_db = wy_base::load_model('module_model');
		$this->userid = $_SESSION['userid'];
		wy_base::load_app_class('ku6api', 'video', 0);
		wy_base::load_app_class('v', 'video', 0);
	...
	public function add() {
		if ($_POST['dosubmit']) {
			...
			if((empty($this->setting['sn']) || empty($this->setting['skey'])) && ROUTE_A!='open') {
				header("Location: ".APP_PATH."index.php?m=video&c=video&a=open&meunid=".$_GET['meunid'].'&wy_ssud='.$_GET['wy_ssud']);
			}
	...
	public function public_update_content() {
		...
		$url = wy_base::load_app_class('url', 'content');
	...
	public function public_get_pos () {
		...
		$category = wy_base::load_model('category_model');
	...
	*/
	public function get_category(){
		...
		$sitemodel_field = wy_base::load_model('sitemodel_field_model');
		...
		$cat_db = wy_base::load_model('category_model');
		if ($models) {
			...
			if (is_array($result)) { 
				...
				$data = $return_data = $categorys = array(); 
				$tree = wy_base::load_sys_class('tree');//factory::load_class('tree', 'utils');
	...
	public function public_check_status() {
		...
		$status_arr = wy_base::load_config('ku6status_config');
		if ($return['ku6status'] != $r['status']) {
			...
			if ($return['ku6status']==21) {
				...
				if (is_array($result) && !empty($result)) {
					...
					$html = wy_base::load_app_class('html', 'content');
					$content_db = wy_base::load_model('content_model');
					$content_check_db = wy_base::load_model('content_check_model');
					$url = wy_base::load_app_class('url', 'content');
					foreach ($result as $rs) {
						$modelid = intval($rs['modelid']);
						$contentid = intval($rs['contentid']);
		...
		$this->setting = getcache('video');
 		if(empty($this->setting['skey']) || empty($this->setting['sn'])){
			...
			if ($return_check==200 && SITE_URL != 'localhost' && !preg_match("/^(127|192|10)\.([1-2]?)([0-9]?)([0-9])\.([1-2]?)([0-9]?)([0-9])\.([1-2]?)([0-9]?)([0-9])/", SITE_URL)) {//存在同域名记录，进行email验证
				header("Location: ".APP_PATH."index.php?m=video&c=video&a=check_user_back&meunid=".$_GET['meunid'].'&wy_ssud='.$_GET['wy_ssud']);
				exit;
			}
