...
<form name="myform" action="?m=video&c=video&a=check_user_back&wy_ssud=<?php echo $_GET['wy_ssud'];?>" method="post" id="myform">
<fieldset>
...
<!--填写skey_sn-->
<br>
<form name="myform2" action="?m=video&c=video&a=setting&wy_ssud=<?php echo $_GET['wy_ssud'];?>" method="post" id="myform2">
<fieldset>
...
function send_code() {
	var email = $("#email").val(); 
	var wy_ssud = "<?php echo $_GET['wy_ssud'];?>";
	if(email==''){
		alert('email 不能为空！');return false;
	}
	$.get('?m=video&c=video&a=send_code&wy_ssud='+wy_ssud,{ email:email,random:Math.random()}, function(data){
		if(data==1) { 
