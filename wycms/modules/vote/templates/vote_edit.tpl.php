...
<tr> 
	...
	<tr>
		...
		<script type="text/javascript">$.getJSON('?m=admin&c=category&a=public_tpl_file_list&style=<?php echo $info['default_style']?>&module=vote&templates=vote_tp&id=<?php echo $template?>&name=vote_subject&wy_ssud='+wy_ssud, function(data){$('#show_template').html(data.vote_tp_template);});</script>
		</td>
	</tr>
...
function del_old(id) {
	$.get('?m=vote&c=vote&a=del_option&optionid='+id+'&wy_ssud='+wy_ssud,null,function (msg) { 
	if (msg==1) { 
...
function load_file_list(id) {
	$.getJSON('?m=admin&c=category&a=public_tpl_file_list&style='+id+'&module=vote&templates=vote_tp&name=subject&wy_ssud='+wy_ssud, function(data){$('#show_template').html(data.vote_tp_template);});
}
