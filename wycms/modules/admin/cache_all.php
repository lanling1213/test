...
class cache_all extends admin {
	...
	public function init() {
		if (isset($_POST['dosubmit']) || isset($_GET['dosubmit'])) {
			...
			$this->cache_api = wy_base::load_app_class('cache_api', 'admin');
			$m = $modules[$page];
			if ($m['mod'] && $m['function']) {
				if ($m['file'] == '') $m['file'] = $m['function'];
				$M = getcache('modules', 'commons');
				if (in_array($m['mod'], array_keys($M))) {
					$cache = wy_base::load_app_class($m['file'], $m['mod']);
					$cache->{$m['function']}();
				}
			...
			if (!empty($modules[$page])) {
				...
				showmessage(L('update').$m['name'].L('cache_file_success'), '?m=admin&c=cache_all&page='.$page.'&dosubmit=1&wy_ssud='.$_SESSION['wy_ssud'], 0);
			} else {
