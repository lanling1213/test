...
<!--
	$(function(){
		...
		$("#wysso_api_url").formValidator({onshow:"<?php echo L('setting_wysso_type')?>",onfocus:"<?php echo L('setting_wysso_type')?>",tipcss:{width:'300px'},empty:false}).inputValidator({onerror:"<?php echo L('setting_wysso_type')?>"}).regexValidator({regexp:"http:\/\/(.+)[^/]$",onerror:"<?php echo L('setting_wysso_type')?>"});
		
		$("#wysso_appid").formValidator({onshow:"<?php echo L('input').L('setting_wysso_appid')?>",onfocus:"<?php echo L('input').L('setting_wysso_appid')?>"}).regexValidator({regexp:"^\\d{1,8}$",onerror:"<?php echo L('setting_wysso_appid').L('must_be_number')?>"});
		$("#wysso_version").formValidator({onshow:"<?php echo L('input').L('setting_wysso_version')?>",onfocus:"<?php echo L('input').L('setting_wysso_version')?>"}).regexValidator({regexp:"^\\d{1,8}$",onerror:"<?php echo L('setting_wysso_version').L('must_be_number')?>"});
		$("#wysso_auth_key").formValidator({onshow:"<?php echo L('input').L('setting_wysso_auth_key')?>",onfocus:"<?php echo L('input').L('setting_wysso_auth_key')?>"}).regexValidator({regexp:"^\\w{32}$",onerror:"<?php echo L('setting_wysso_auth_key').L('must_be_32_w')?>"});
	})
//-->
</script>
...
<div id="div_setting_2" class="contentList pad-10 hidden">
  ...
  <tr>
    <th><?php echo L('geetest_open')?></th>
    <td class="y-bg">
    <table>
    <tr>
        <td>
            <input onclick="showsmtp(this, 'geetest_settings')" name="setconfig[geetest_open]" value="1" type="radio" <?php echo ($geetest_open=='1') ? ' checked' : ''?>> <?php echo L('setting_yes')?>&nbsp;&nbsp;&nbsp;&nbsp;
            <input onclick="showsmtp(this, 'geetest_settings')" name="setconfig[geetest_open]" value="0" type="radio" <?php echo ($geetest_open=='0' || !$geetest_open) ? ' checked' : ''?>> <?php echo L('setting_no')?>
            &nbsp;&nbsp;<font style="color:#999;">(第三方验证码，安全性更高，用户体验更佳。)</font>
        </td>
    </tr>
    <tr id="geetest_settings" <?php echo ($geetest_open == 0) ?'style="display:none"' :''; ?>><td>
        <table>
        <tr>
            <th><?php echo L('geetest_id'); ?>：</th>
            <td><input style="width:300px" type="text" class="input-text" name="setconfig[geetest_id]" id="admin_url" size="30" value="<?php echo $geetest_id?>"/></td>
        </tr>
        <tr>
            <th><?php echo L('geetest_key'); ?>：</th>
            <td><input style="width:300px" type="text" class="input-text" name="setconfig[geetest_key]" id="admin_url" size="30" value="<?php echo $geetest_key?>"/></td>
        </tr>
        </table>
    </td></tr>
    </table>
...
<table width="100%"  class="table_form">
  <tr>
    <th width="120"><?php echo L('setting_wysso')?></th>
    <td class="y-bg">
    <input name="setconfig[wysso]" value="1" type="radio"  <?php echo ($wysso=='1') ? ' checked' : ''?>> <?php echo L('setting_yes')?>&nbsp;&nbsp;&nbsp;&nbsp;
	 <input name="setconfig[wysso]" value="0" type="radio"  <?php echo ($wysso=='0') ? ' checked' : ''?>> <?php echo L('setting_no')?></td>
  </tr> 
  <tr>
    <th><?php echo L('setting_wysso_appid')?></th>
    <td class="y-bg"><input type="text" class="input-text" name="setconfig[wysso_appid]" id="wysso_appid" size="30" value="<?php echo $wysso_appid ?>"/></td>
  </tr> 
  <tr>
    <th><?php echo L('setting_wysso_wysso_api_url')?></th>
    <td class="y-bg"><input type="text" class="input-text" name="setconfig[wysso_api_url]" id="wysso_api_url" size="50" value="<?php echo $wysso_api_url ?>"/></td>
  </tr>  
   <tr>
    <th><?php echo L('setting_wysso_auth_key')?></th>
    <td class="y-bg"><input type="text" class="input-text" name="setconfig[wysso_auth_key]" id="wysso_auth_key" size="50" value="<?php echo $wysso_auth_key ?>"/></td>
  </tr>
   <tr>
    <th><?php echo L('setting_wysso_version')?></th>
    <td class="y-bg"><input type="text" class="input-text" name="setconfig[wysso_version]" id="wysso_version" size="2" value="<?php echo $wysso_version ?>"/></td>
  </tr>      
  </table>
</div>
