...
include WY_PATH.'modules'.DIRECTORY_SEPARATOR.'admin'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.'header.tpl.php';
?>
...
<?php } ?>
<?php if(wy_base::load_config('system','debug')) {?>
<?php echo L('main_safety_debug')?><br />
<?php } ?>
<?php if(!wy_base::load_config('system','errorlog')) {?>
<?php echo L('main_safety_errlog')?><br />
<?php } ?>
	<div class="bk20 hr"><hr /></div>	
<?php if(wy_base::load_config('system','execution_sql')) {?>	
...
<?php } ?>
<?php 
$tpl_edit = wy_base::load_config('system','tpl_edit');
if($tpl_edit=='1') {?>
...
if(module_exists('member') && is_array($ccache)) { ?>
	...
	<?php foreach($adminpanel as $v) {?>
		<span>
			[<a target="right" href="<?php echo $v['url'].'&menuid='.$v['menuid'];?>&wy_ssud=<?php echo $_SESSION['wy_ssud'];?>"><?php echo L($v['name'])?></a>]   
		</span>
	<?php }?>
	</div>
<?php } else { ?>
...
<div class="pad-10">
<form action="?m=admin&c=cache_all&a=init&wy_ssud=<?php echo $_SESSION['wy_ssud'];?>" target="cache_if" method="post" id="myform" name="myform">
...
<div class="col-2 col-auto">
	...
	<div class="content">
	<?php echo L('main_version')?>WYcms <?php echo WY_VERSION?>  Release <?php echo WY_RELEASE?></a>]<br />
...
<div class="col-2 lf mr10" style="width:48%">
	...
	<?php echo L('main_product_site')?><a href="http://www.lawyermarketing.cn/" target="_blank">http://www.lawyermarketing.cn/</a> <br />
	<?php echo L('main_product_bbs')?><a href="http://bbs.lawyermarketing.cn/" target="_blank">http://bbs.lawyermarketing.cn/</a> <br />
	<?php echo L('main_product_sales')?>2851026691
	</div>
</div>
<div class="bk10"></div>
</div>
