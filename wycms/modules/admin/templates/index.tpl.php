...
var wy_ssud = '<?php echo $_SESSION['wy_ssud']?>'
</script>
<style type="text/css">
...
<?php $model_types = wy_base::load_config('model_config');?>
...
<div class="header">
	<div class="logo lf"><a href="<?php echo $currentsite['domain']?>" target="_blank"><span class="invisible"><?php echo L('wycms_title')?></span></a></div>
    <div class="rt-col">
...
<div id="content">
    ...
    <div class="crumbs">
    <div class="shortcut cu-span"><a href="?m=content&c=create_html&a=public_index&wy_ssud=<?php echo $_SESSION['wy_ssud'];?>" target="right"><span><?php echo L('create_index')?></span></a><a href="?m=admin&c=cache_all&a=init&wy_ssud=<?php echo $_SESSION['wy_ssud'];?>" target="right"><span><?php echo L('update_backup')?></span></a><a href="javascript:art.dialog({id:'map',iframe:'?m=admin&c=index&a=public_map', title:'<?php echo L('background_map')?>', width:'700', height:'500', lock:true});void(0);"><span><?php echo L('background_map')?></span></a><?php echo runhook('admin_top_left_menu')?></div>
                ...
                <div class="fav-nav">
					<div id="panellist">
						<?php foreach($adminpanel as $v) {?>
								<span>
								<a onclick="paneladdclass(this);" target="right" href="<?php echo $v['url'].'menuid='.$v['menuid'].'&wy_ssud='.$_SESSION['wy_ssud'];?>"><?php echo L($v['name'])?></a>
...
function _M(menuid,targetUrl) {
	...
	if(menuid!=8) {
		...
		$("#leftMain").load("?m=admin&c=wysso&a=public_menu_left&menuid="+menuid, {limit: 25}, function(){
		   windowW();
		 });
	}
...
function _MP(menuid,targetUrl) {
	...
	$("#rightMain").attr('src', targetUrl+'&menuid='+menuid+'&wy_ssud='+wy_ssud);
...
function show_help(targetUrl) {
	$("#help").slideUp("slow");
	var str = '';
	$.getJSON("http://help.wycms.cn/api.php?jsoncallback=?",{op:'help',targetUrl: targetUrl},
	function(data){
		if(data!=null) {
			$("#help").slideDown("slow");
