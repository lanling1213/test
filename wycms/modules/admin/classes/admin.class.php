...
class admin {
	...
	public function __construct() {
		self::check_admin();
		self::check_priv();
		wy_base::load_app_func('global','admin');
		if (!module_exists(ROUTE_M)) showmessage(L('module_not_exists'));
		self::manage_log();
		self::check_ip();
		self::lock_screen();
		self::check_hash();
		if(wy_base::load_config('system','admin_url') && $_SERVER["HTTP_HOST"]!= wy_base::load_config('system','admin_url')) {
	...
	final public static function admin_tpl($file, $m = '') {
		...
		return WY_PATH.'modules'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$file.'.tpl.php';
	}
	...
	final public static function admin_menu($parentid, $with_self = 0) {
		...
		$array = array();
		$privdb = wy_base::load_model('admin_role_priv_model');
	...
	final public static function submenu($parentid = '', $big_menu = false) {
		...
		foreach($array as $_value) {
			...
			if($classname) {
				...
				$string .= "<a href='?m=".$_value['m']."&c=".$_value['c']."&a=".$_value['a']."&menuid=$parentid&wy_ssud=$wy_ssud".'&'.$_value['data']."' $classname><em>".L($_value['name'])."</em></a><span>|</span>";
			}
		}
	...
	final public static function get_siteinfo($siteid = '') {
		...
		$sites = wy_base::load_app_class('sites', 'admin');
	...
	final private function manage_log() {
		//判断是否记录
		$setconfig = wy_base::load_config('system');
		extract($setconfig);
 		if($admin_log==1){
 			$action = ROUTE_A;
 			if($action == '' || strchr($action,'public') || $action == 'init' || $action=='public_current_pos') {
				...
				$ip = ip();
				$log = wy_base::load_model('log_model');
	...
	final private function check_hash() {
		...
		if(isset($_GET['wy_ssud']) && $_SESSION['wy_ssud'] != '' && ($_SESSION['wy_ssud'] == $_GET['wy_ssud'])) {
			return true;
		} elseif(isset($_POST['wy_ssud']) && $_SESSION['wy_ssud'] != '' && ($_SESSION['wy_ssud'] == $_POST['wy_ssud'])) {
			return true;
		} else {
	...
	final public function admin_list_template($id = '', $str = '') {
		$templatedir = WY_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.'content'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR;
		$pre = 'content_list';
