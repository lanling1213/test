...
class module_api {
	...
	public function __construct() {
		$this->db = wy_base::load_model('module_model');
	}
	...
	public function install($module = '') {
		...
		$this->installdir = WY_PATH.'modules'.DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR.'install'.DIRECTORY_SEPARATOR;
		...
		if (is_array($models) && !empty($models)) {
			foreach ($models as $m) {
				$this->m_db = wy_base::load_model($m.'_model');
				$sql = file_get_contents($this->installdir.$m.'.sql');
				$this->sql_execute($sql);
			}
		}
		if (file_exists($this->installdir.'extention.inc.php')) {
			$menu_db = wy_base::load_model('menu_model');
			@include ($this->installdir.'extention.inc.php');
			if(!defined('INSTALL_MODULE')) {
				$file = WY_PATH.'languages'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.'system_menu.lang.php';
				if(file_exists($file)) {
		...
		if(!defined('INSTALL_MODULE')) {
			if (file_exists($this->installdir.'languages'.DIRECTORY_SEPARATOR)) {
				dir_copy($this->installdir.'languages'.DIRECTORY_SEPARATOR, WY_PATH.'languages'.DIRECTORY_SEPARATOR);
			}
			if(file_exists($this->installdir.'templates'.DIRECTORY_SEPARATOR)) {
				dir_copy($this->installdir.'templates'.DIRECTORY_SEPARATOR, WY_PATH.'templates'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR);
				if (file_exists($this->installdir.'templates'.DIRECTORY_SEPARATOR.'name.inc.php')) {
					$keyid = 'templates|'.wy_base::load_config('system', 'tpl_name').'|'.$this->module;
					$file_explan[$keyid] = include $this->installdir.'templates'.DIRECTORY_SEPARATOR.'name.inc.php';
					$templatepath = WY_PATH.'templates'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR;
					...
					unlink(WY_PATH.'templates'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR.'name.inc.php');
				}
			}
		}
		return true;
	...
	define('INSTALL', true);
		...
		if(!defined('INSTALL_MODULE')) {
			if (dir_create(WY_PATH.'languages'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.'test_create_dir')) {
				sleep(1);
				dir_delete(WY_PATH.'languages'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.'test_create_dir');
		...
		if (is_array($models) && !empty($models)) {
			foreach ($models as $m) {
				if (!file_exists(WY_PATH.'model'.DIRECTORY_SEPARATOR.$m.'_model.class.php')) {
	...
	public function uninstall($module) {
		...
		$this->module = $module;
		$this->uninstalldir = WY_PATH.'modules'.DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR.'uninstall'.DIRECTORY_SEPARATOR;
		...
		if (file_exists(WY_PATH.'languages'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.$this->module.'.lang.php')) {
			@unlink(WY_PATH.'languages'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.$this->module.'.lang.php');
		}
		if (is_dir(WY_PATH.'templates'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.$this->module)) {
			@dir_delete(WY_PATH.'templates'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.$this->module);
		}
		$templatepath = WY_PATH.'templates'.DIRECTORY_SEPARATOR.wy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR;
		if (file_exists($templatepath.'config.php')) {
			$keyid = 'templates|'.wy_base::load_config('system', 'tpl_name').'|'.$this->module;
	...
	private function sql_split($sql) {
		...
		if (!$dbcharset) {
			$dbcharset = wy_base::load_config('database','default');
		...
		if($this->m_db->db_tablepre != "wy_") $sql = str_replace("wy_", $this->m_db->db_tablepre, $sql);
