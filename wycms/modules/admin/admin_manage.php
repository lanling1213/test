...
class admin_manage extends admin {
	...
	function __construct() {
		...
		$this->role_db = wy_base::load_model('admin_role_model');
		$this->op = wy_base::load_app_class('admin_op');
	}
	...
	public function public_edit_info() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$lang_dirs = glob(WY_PATH.'languages/*');
			$dir_array = array();
			foreach($lang_dirs as $dirs) {
				$dir_array[] = str_replace(WY_PATH.'languages/','',$dirs);
			}
	...
	//电子口令卡
	function card() {
		if (wy_base::load_config('system', 'safe_card') != 1) {
	...
	private function check_admin_manage_code(){
		...
		$wy_auth_key = md5(wy_base::load_config('system','auth_key').'adminuser');
		$admin_manage_code = sys_auth($admin_manage_code, 'DECODE', $wy_auth_key);	
		if($admin_manage_code==""){
			return false;
		}
		$admin_manage_code = explode("_", $admin_manage_code);
		if($admin_manage_code[0]!="adminuser" || $admin_manage_code[1]!=$_POST[wy_ssud]){
			return false;
		}
		return true;
	}
	...
	private function get_admin_manage_code(){
		$wy_auth_key = md5(wy_base::load_config('system','auth_key').'adminuser');
		$code = sys_auth("adminuser_".$_GET[wy_ssud]."_".time(), 'ENCODE', $wy_auth_key);
		return $code;
	}	
}
?>
