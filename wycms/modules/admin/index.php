...
class index extends admin {
	public function __construct() {
		parent::__construct();
		$this->db = wy_base::load_model('admin_model');
		$this->menu_db = wy_base::load_model('menu_model');
		$this->panel_db = wy_base::load_model('admin_panel_model');
	}
	...
	public function init () {
		...
		$site = wy_base::load_app_class('sites');
	...
	public function login() {
		...
		if(isset($_GET['dosubmit'])) {
			...
			//查看是否使用口令卡
			if (!isset($_GET['card']) && $r['card'] && wy_base::load_config('system', 'safe_card') == 1) {
				$_SESSION['card_username'] = $username;
				$_SESSION['card_password'] = $_POST['password'];
				header("location:?m=admin&c=index&a=public_card");
				exit;
			} elseif (isset($_GET['card']) && wy_base::load_config('system', 'safe_card') == 1 && $r['card']) {//对口令卡进行验证
			...
			$_SESSION['wy_ssud'] = random(6,'abcdefghigklmnopqrstuvwxwyABCDEFGHIGKLMNOPQRSTUVWXWY0123456789');
			...
			if ($video_setting['sn'] && $video_setting['skey']) {
				$vmsapi = wy_base::load_app_class('ku6api', 'video');
	...
	public function public_card() {
		...
		if(!$r) showmessage(L('user_not_exist'),'?m=admin&c=index&a=login');
		if (isset($_GET['dosubmit'])) {
			wy_base::load_app_class('card', 'admin', 0);
	...
	public function public_logout() {
		...
		$wysso_logout = '<script type="text/javascript" src="'.$wysso_api_url.'/api.php?op=logout" reload="1"></script>';
		
		showmessage(L('logout_success').$wysso_logout,'?m=admin&c=index&a=login');
	}
	...
	public function public_ajax_add_panel() {
		...
		foreach($panelarr as $v) {
			echo "<span><a onclick='paneladdclass(this);' target='right' href='".$v['url'].'&menuid='.$v['menuid']."&wy_ssud=".$_SESSION['wy_ssud']."'>".L($v['name'])."</a>  <a class='panel-delete' href='javascript:delete_panel(".$v['menuid'].");'></a></span>";
		}
		exit;
	}
	...
	public function public_ajax_delete_panel() {
		...
		$panelarr = $this->panel_db->listinfo(array('userid'=>$_SESSION['userid']), "datetime");
		foreach($panelarr as $v) {
			echo "<span><a onclick='paneladdclass(this);' target='right' href='".$v['url']."&wy_ssud=".$_SESSION['wy_ssud']."'>".L($v['name'])."</a> <a class='panel-delete' href='javascript:delete_panel(".$v['menuid'].");'></a></span>";
		}
		exit;
	}
	public function public_main() {
		wy_base::load_app_func('global');
		wy_base::load_app_func('admin');
		define('WY_VERSION', wy_base::load_config('version','wy_version'));
		define('WY_RELEASE', wy_base::load_config('version','wy_release'));	
		...
		$show_header = $show_wy_ssud = 1;
		/*检测框架目录可写性*/
		$wy_writeable = is_writable(WY_PATH.'base.php');
		$common_cache = getcache('common','commons');
		$logsize_warning = errorlog_size() > $common_cache['errorlog_size'] ? '1' : '0';
		$adminpanel = $this->panel_db->select(array('userid'=>$userid), '*',20 , 'datetime');
		$product_copyright = '广州网律互联网科技有限公司';
		$programmer = '米建、李晚辉、黄功强、王靖';
 		$designer = '刘先告';
		ob_start();
	...
	public function public_snda_status() {
		//引入盛大接口
		if(!strstr(wy_base::load_config('snda','snda_status'), '|')) {
			$this->site_db = wy_base::load_model('site_model');
			$uuid_arr = $this->site_db->get_one(array('siteid'=>1), 'uuid');
			$uuid = $uuid_arr['uuid'];
			$snda_check_url = "http://open.sdo.com/wycms?cmsid=".$uuid."&sitedomain=".$_SERVER['SERVER_NAME'];
	...
	public function public_set_model() {
		...
		if (!$model) {
			...
			$models = wy_base::load_config('model_config');
		...
		$menudb = wy_base::load_model('menu_model');
