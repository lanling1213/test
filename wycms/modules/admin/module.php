...
class module extends admin {
	...
	public function init() {
		...
		$dirs = glob(WY_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.'*');
		foreach ($dirs as $d) {
			if (is_dir($d)) {
				$d = basename($d);
				$dirs_arr[] = $d;
	...
	public function install() {
		...
		$module_api = wy_base::load_app_class('module_api');
		if (!$module_api->check($this->module)) showmessage($module_api->error_msg, 'blank');
		if ($_POST['dosubmit']) {
			if ($module_api->install()) showmessage(L('success_module_install').L('update_cache'), '?m=admin&c=module&a=cache&wy_ssud='.$_SESSION['wy_ssud']);
			else showmesage($module_api->error_msg, HTTP_REFERER);
		} else {
			include WY_PATH.'modules'.DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR.'install'.DIRECTORY_SEPARATOR.'config.inc.php';
	...
	public function uninstall() {
		...
		else showmessage(L('uninstall_success'), '?m=admin&c=module&a=cache&wy_ssud='.$_SESSION['wy_ssud']);
	}
	...
	public function cache() {
		echo '<script type="text/javascript">parent.right.location.href = \'?m=admin&c=cache_all&a=init&wy_ssud='.$_SESSION['wy_ssud'].'\';window.top.art.dialog({id:\'install\'}).close();</script>';
