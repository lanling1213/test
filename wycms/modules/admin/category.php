...
class category extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = wy_base::load_model('category_model');
		...
		if(!empty($result)) {
			foreach($result as $r) {
				...
				if(!$show_detail) {
					...
					$r['parentid'] = 0;
					$r['str_manage'] .= '<a href="?m=admin&c=category&a=init&parentid='.$r['catid'].'&menuid='.$_GET['menuid'].'&s='.$r['type'].'&wy_ssud='.$_SESSION['wy_ssud'].'">'.L('manage_sub_category').'</a> | ';
				}
				$r['str_manage'] .= '<a href="?m=admin&c=category&a=add&parentid='.$r['catid'].'&menuid='.$_GET['menuid'].'&s='.$r['type'].'&wy_ssud='.$_SESSION['wy_ssud'].'">'.L('add_sub_category').'</a> | ';
				
				$r['str_manage'] .= '<a href="?m=admin&c=category&a=edit&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'&type='.$r['type'].'&wy_ssud='.$_SESSION['wy_ssud'].'">'.L('edit').'</a> | <a href="javascript:confirmurl(\'?m=admin&c=category&a=delete&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'\',\''.L('confirm',array('message'=>addslashes($r['catname']))).'\')">'.L('delete').'</a> | <a href="?m=admin&c=category&a=remove&catid='.$r['catid'].'&wy_ssud='.$_SESSION['wy_ssud'].'">'.L('remove','','content').'</a>';
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['template_child']){
                                ...
                                $idstr = $this->get_arrchildid($catid);
                                 if(!empty($idstr)){
                                        $sql = "select catid,setting from wycms_category where catid in($idstr)";
                                        $this->db->query($sql);
			...
			$this->cache();
			//更新附件状态
			if($_POST['info']['image'] && wy_base::load_config('system','attachment_stat')) {
	...
	private function delete_category_video($catid, $modelid) {
		$content_model = wy_base::load_model('content_model');
	...
	*/
	private function update_url($catid) {
		...
		$url = wy_base::load_app_class('url', 'content'); //调用URL实例
	...
	public function count_items() {
		$this->content_db = wy_base::load_model('content_model');
		$result = getcache('category_content_'.$this->siteid,'commons');
		foreach($result as $r) {
			if($r['type'] == 0) {
				$modelid = $r['modelid'];
	...
	public function batch_edit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			foreach ($infos as $catid=>$info) {
				...
				//更新附件状态
				if($info['image'] && wy_base::load_config('system','attachment_stat')) {
