...
class database extends admin {
	...
	function __construct() {
		...
		wy_base::load_sys_class('db_factory');
	...
	public function import() {
		$database = wy_base::load_config('database');
		if($_GET['dosubmit']) {
			$admin_founders = explode(',',wy_base::load_config('system','admin_founders'));
			...
			if(is_array($sqlfiles)) {
				...
				foreach($sqlfiles as $id=>$sqlfile) {
					//老的数据库备份文件转换为新格式
					if(preg_match("/([wycmstables_|db_][0-9]{8}_[0-9a-z]{20}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
					...
					if(preg_match("/([0-9a-z]{20}_[wycmstables_|db_]+[0-9]{8}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
	...
	private function status($tables,$tablepre) {
		...
		return array('wycmstables'=>$wycms, 'othertables'=>$other);
	}
	 ...
	 * @param unknown_type $tabletype 备份数据库类型 （非wycms数据与wycms数据）
	 */
	private function export_database($tables,$sqlcompat,$sqlcharset,$sizelimit,$action,$fileid,$random,$tableid,$startfrom,$tabletype) {
		...
		if(trim($tabledump)) {
			$tabledump = "# wycms bakfile\n# version:WYCMS V9\n# time:".date('Y-m-d H:i:s')."\n# type:wycms\n# wycms:http://www.wycms.cn\n# --------------------------------------------------------\n\n\n".$tabledump;
			$tableid = $i;
	 ...
	 */
 	...
 	private function sql_split($sql) {
		...
		if($this->db_tablepre != "wycms_") $sql = str_replace("`wycms_", '`'.$this->db_tablepre, $sql);
		$sql = str_replace("\r", "\n", $sql);
		$ret = array();
		$num = 0;
