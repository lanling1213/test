...
class role extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = wy_base::load_model('admin_role_model');
		$this->priv_db = wy_base::load_model('admin_role_priv_model');
		$this->op = wy_base::load_app_class('role_op');
	}
	...
	public function role_priv() {
		...
		if(isset($_POST['dosubmit'])){
			...
			$roleid = intval($_GET['roleid']);
			if ($siteid) {
				$menu = wy_base::load_sys_class('tree');
				$menu->icon = array('│ ','├─ ','└─ ');
				$menu->nbsp = '&nbsp;&nbsp;&nbsp;';
	...
	public function member_manage() {
		$this->admin_db = wy_base::load_model('admin_model');
		$roleid = intval($_GET['roleid']);
	...
	public function setting_cat_priv() {
		...
		switch ($op) {
			...
			wy_base::load_app_class('role_cat', '', 0);
