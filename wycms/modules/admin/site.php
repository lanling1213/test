...
class site extends admin {
	...
	public function __construct() {
		$this->db = wy_base::load_model('site_model');
		parent::__construct();
	}
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->db->insert(array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template,'setting'=>$setting, 'default_style'=>$default_style))) {
				$class_site = wy_base::load_app_class('sites');
			...
			$release_point_db = wy_base::load_model('release_point_model');
	...
	public function edit() {
		...
		if ($data = $this->db->get_one(array('siteid'=>$siteid))) {
			if (isset($_POST['dosubmit'])) {
				...
				$_POST['setting']['watermark_img'] = 'skins/images/water/'.$_POST['setting']['watermark_img'];
				$setting = trim(array2string($_POST['setting']));
				...
				$setting['watermark_img'] = str_replace('skins/images/water/','',$setting['watermark_img']);
				$release_point_db = wy_base::load_model('release_point_model');
				$release_point_list = $release_point_db->select('', 'id, name');
    ...
    {
        $siteid = get_siteid();
		$siteid = intval($siteid) ? intval($siteid) : showmessage(L('illegal_parameters'), HTTP_REFERER);
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $infos = array2string($_POST['info']);
            if ($this->db->update(array('infos'=> $infos), array('siteid'=>$siteid))) {
                $class_site = wy_base::load_app_class('sites');
                $class_site->set_cache();
            ...
            $infos = string2array($data['infos']);
            extract($infos);

            $show_validator = true;
            $show_header    = true;
            include $this->admin_tpl('site_edit_infos');
        }
    }
	...
	public function public_dirname() {
		$dirname = isset($_GET['dirname']) && trim($_GET['dirname']) ? (wy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['dirname'])) : trim($_GET['dirname'])) : exit('0');
		$siteid = isset($_GET['siteid']) && intval($_GET['siteid']) ? intval($_GET['siteid']) : '';
		$data = array();
		if ($siteid) {
