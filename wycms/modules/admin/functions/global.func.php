<?php 
	...
	function set_config($config, $filename="system") {
		...
		foreach($config as $k=>$v) {
			if(in_array($k,array('js_path','css_path','img_path','attachment_stat','admin_log','gzip','errorlog','wysso','wysso_appid','wysso_api_url','wysso_auth_key','wysso_version','connect_enable', 'upload_url','sina_akey', 'sina_skey', 'snda_enable', 'snda_status', 'snda_akey', 'snda_skey', 'qq_akey', 'qq_skey','qq_appid','qq_appkey','qq_callback','admin_url','geetest_open','geetest_id','geetest_key'))) {
				$v = trim($v);
				$configs[$k] = $v;
		...
		return wy_base::load_config('system','lock_ex') ? file_put_contents($configfile, $str, LOCK_EX) : file_put_contents($configfile, $str);		
	}
	...
	function errorlog_size() {
		...
		if(file_exists($logfile)) {
			return $logsize = wy_base::load_config('system','errorlog') ? round(filesize($logfile) / 1048576 * 100) / 100 : 0;
		} 
		return 0;
	}
