 ...
 * @param $module 模型名
 */
function wy_tag_class ($module) {
	$filepath = WY_PATH.'modules'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$module.'_tag.class.php';
	if (file_exists($filepath)) {
		$wy_tag = wy_base::load_app_class($module.'_tag', $module); 
		if (!method_exists($wy_tag, 'wy_tag')) {
...
function template_url($id) {
	...
	if (!file_exists($filepath)) {
		$datacall = wy_base::load_model('datacall_model');
