...
<form name="myform" action="?m=ask&c=position&a=listorder" method="post">
<div class="pad_10">
<div class="table-list">
    <table width="100%" cellspacing="0">
        <thead>
            ...
            <th width="15%"><?php echo L('posid_typeid');?></th>
            <th width="8%"><?php echo L('position_num');?></th>
...
?>   
	...
	<td width="15%" align="center"><?php echo $info['typeid'] ? $TYPES[$info['typeid']]['typename'] : L('posid_all')?></td>
	<td  width="8%" align="center"><?php echo $info['num']?></td>
	<td width="20%" align="center">
	<a href="?m=ask&c=position&a=public_item&posid=<?php echo $info['posid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo L('posid_item_manage')?></a> |
	...
	<?php } else {?>
	<a href="javascript:confirmurl('?m=ask&c=position&a=delete&posid=<?php echo $info['posid']?>', '<?php echo L('posid_del_cofirm')?>')"><?php echo L('delete')?></a>	
	<?php } ?>
	| 
	<?php if($info['thumb']!=""){?>
...
<!--
	...
	window.top.art.dialog({title:'<?php echo L('edit')?>--'+name, id:'edit', iframe:'?m=ask&c=position&a=edit&posid='+id ,width:'500px',height:'360px'}, 	function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;
...
function preview(thumb, name) {
	...
	window.top.art.dialog({title:'预览 '+name+' ',id:'preview',iframe:'?m=ask&c=position&a=preview&thumb='+thumb,width:'530',height:'400'}, '', function(){window.top.art.dialog({id:'preview'}).close()});
}
//-->
</script>
