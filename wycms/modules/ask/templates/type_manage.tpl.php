<?php
defined('IN_ADMIN') or exit('No permission resources.');
include $this->admin_tpl('header', 'admin');?>
<form name="myform" action="?m=ask&c=ask&a=type_listorder&dosubmit=1" method="post">
<div class="pad_10">
<div class="explain-col">
<?php echo L('manage_type');?>，<a href="?m=ask&c=ask&a=public_cache&menuid=<?php echo $_GET['menuid'];?>&module=admin"><?php echo L('update_cache');?></a>
</div>
<div class="bk10"></div>
<div class="table-list">
    <table width="100%" cellspacing="0" >
        <thead>
            <tr>
            <th width="70"><?php echo L('listorder');?></th>
            <th width="60">typeid</th>
            <th ><?php echo L('typename');?></th>
            <th align='center' width="40"><?php echo L('items');?></th>
            <th width="250" ><?php echo L('operations_manage');?></th>
            </tr>
        </thead>
    <tbody>
    <?php echo $all_types;?>
    </tbody>
    </table>

    <div class="btn">
    <input type="hidden" name="wy_ssud" value="<?php echo $_SESSION['wy_ssud'];?>" />
    <input type="submit" class="button" name="dosubmit" value="<?php echo L('listorder')?>" /></div>  </div>
</div>
</div>
</form>
<script language="JavaScript">
<!--
    window.top.$('#display_center_id').css('display','none');
//-->
</script>
</body>
</html>
