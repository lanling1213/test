...
class html {
	...
	public function __construct() {
		$this->db = wy_base::load_model('special_model'); //专题数据模型
		$this->type_db = wy_base::load_model('type_model'); //专题分类数据模型
		$this->c_db = wy_base::load_model('special_content_model'); //专题内容数据模型
		$this->data_db = wy_base::load_model('special_c_data_model'); 
		$this->site = wy_base::load_app_class('sites', 'admin');
	...
	public function _create_content($contentid = 0) {
		...
		wy_base::load_app_func('global', 'special');
		...
		if ($arr_content['paginationtype']) {			//文章使用分页时
			if($arr_content['paginationtype']==1) {
				...
				$contentpage = wy_base::load_app_class('contentpage', 'content'); //调用自动分页类
		...
		if ($CONTENT_POS !== false) {
			...
			for ($i=1; $i<=$pagenumber; $i++) {
				...
				wy_base::load_app_func('util', 'content');
	...
	public function _index($specialid = 0, $pagesize = 20, $pages_num = 0) {
		...
		if ($siteid>1) {
			...
			$file = wy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/index.html';
		} else {
			$file = wy_base::load_config('system', 'html_root').'/special/'.$filename.'/index.html';
		}
		if(!$ispage) {
		...
		if($ispage) {
			...
			for ($i=1; $i<=$pages_num; $i++) {
				...
				$file_root = WYCMS_PATH.$file_root;
				ob_start();
				include template('special', $template);
	...
	public function create_list($page = 1) {
		$siteid = get_siteid();
		$site_info = $this->site->get_by_id($siteid);
		define('URLRULE', $site_info['domain'].substr(wy_base::load_config('system', 'html_root'), 1).'/special/index.html~'.$site_info['domain'].substr(wy_base::load_config('system', 'html_root'), 1).'/special/index-{$page}.html');
		//分站时计算路径
		if ($siteid>1) {
			if ($page==1) $file = wy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/index.html';
			else $file = wy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/index-'.$page.'.html';
		} else {
			if ($page==1)  $file = wy_base::load_config('system', 'html_root').'/special/index.html';
			else $file = wy_base::load_config('system', 'html_root').'/special/index-'.$page.'.html';
		}
		$this->queue->add_queue('add', $file, $siteid);
		$file  = WYCMS_PATH.$file;
		ob_start();
	...
	public function create_type($typeid = 0, $page = 1) {
		...
		extract($s_info);
		$site_info = $this->site->get_by_id($siteid);
		define('URLRULE', $site_info['domain'].substr(wy_base::load_config('system', 'html_root'), 1).'/special/{$specialdir}/{$typedir}/type-{$typeid}.html~'.$site_info['domain'].substr(wy_base::load_config('system', 'html_root'), 1).'/special/{$specialdir}/{$typedir}/type-{$typeid}-{$page}.html');
		...
		if ($siteid>1) {
			if ($page==1) $file = wy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/'.$info['typedir'].'/type-'.$typeid.'.html';
			else $file = wy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/'.$info['typedir'].'/type-'.$typeid.'-'.$page.'.html';
		} else {
			if ($page==1) $file = wy_base::load_config('system', 'html_root').'/special/'.$filename.'/'.$info['typedir'].'/type-'.$typeid.'.html';
			else $file = wy_base::load_config('system', 'html_root').'/special/'.$filename.'/'.$info['typedir'].'/type-'.$typeid.'-'.$page.'.html';
		}
		$this->queue->add_queue('add', $file, $siteid);
		$file = WYCMS_PATH.$file;
		ob_start();
