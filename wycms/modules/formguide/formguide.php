...
class formguide extends admin {
	...
	public function __construct() {
		...
		$this->db = wy_base::load_model('sitemodel_model');
	}
	...
	public function add() {
		if (isset($_POST['dosubmit'])) {
			...
			define('MODEL_PATH',WY_PATH.'modules'.DIRECTORY_SEPARATOR.'formguide'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
			$create_sql = file_get_contents(MODEL_PATH.'create.sql');
			$this->m_db = wy_base::load_model('sitemodel_field_model');
			...
			wy_base::load_sys_class('form', '', false);
	...
	public function delete() {
		...
		if (isset($_GET['formid']) && !empty($_GET['formid'])) {
			$formid = intval($_GET['formid']);
			$m_db = wy_base::load_model('sitemodel_field_model');
	...
	private function sql_split($sql) {
		...
		$sql = str_replace("wy_form_table", $this->m_db->db_tablepre.'form_'.$this->tablename, $sql);
		$ret = array();
		$num = 0;
