...
class index {
	function __construct() {
		$this->db = wy_base::load_model('search_model');
	...
	public function init() {
		...
		if(isset($_GET['q'])) {
			...
			if($setting['sphinxenable']) {
				$sphinx = wy_base::load_app_class('search_interface', '', 0);
				...
				wy_base::load_sys_class('segment', '', 0);
				$segment = new segment();
				//分词结果
			...
			if($setting['relationenble']) {
				...
				$this->keyword_db = wy_base::load_model('search_keyword_model');
			  ...
			  if(!empty($result) || !empty($commend['id'])) {
				...
				if($modelid) {
					...
					if(empty($this->content_db->model_tablename)) {
						$this->content_db = wy_base::load_model('yp_content_model');
						$this->content_db->set_model($modelid);
					...
					//读取专辑搜索接口
					$special_api = wy_base::load_app_class('search_api', 'special');
