...
class tag extends admin {
	...
	public function __construct() {
		$this->db = wy_base::load_model('tag_model');
		$this->dbsource = wy_base::load_model('dbsource_model');
		parent::__construct();
	}
	 ...
	 */
	public function add() {
		wy_base::load_app_func('global', 'dbsource');
		if (isset($_POST['dosubmit'])) {
			...
			if ($type == '1') { //自定义SQL
				...
				$html = wy_tag_class($module);
				$data = array();
				$tag = '{wy:'.$module.' action="'.$action.'" ';
			...
			$modules = array_merge(array(''=>L('please_select')),wy_base::load_config('modules'));
			...
			if ($module) $html = wy_tag_class($module);
