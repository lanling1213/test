...
class index extends admin {
	private $_filearr = array('api', 'wycms', 'skins', '');
	//md5验证地址
	private $_upgrademd5 = 'http://www.wycms.cn/upgrademd5/';
	//补丁地址
	private $_patchurl = 'http://download.wycms.cn/patch/';
	...
	public function init() {
		...
		//获取当前版本
		$current_version = wy_base::load_config('version');
		...
		foreach($allpathlist as $k=>$v) {
			if(strstr($v, 'patch_'.$current_version['wy_release'])) {
				$key = $k;
				break;
			}
		}
		...
		if(!empty($_GET['s'])) {
			...
			foreach($pathlist as $k=>$v) {
					...
					foreach ($file_list as $fk=>$fv) {
						if(in_array(strtolower(substr($fv, -3, 3)), array('php', 'sql'))) {
							if (strtolower(substr($file_list[$fk], -3, 3)) == 'sql' && $data = file_get_contents($file_list[$fk])) {
								...
								if (!$db = wy_base::load_model($model_name.'_model')) {
								...
								if (strtolower(basename($file_list[$fk])) == 'system_menu.lang.php' && file_exists($file_list[$fk])) {
									...
									$menu_lan_file = WY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system_menu.lang.php';
				...
				if(file_exists($configpath)) {
					...
					$version_arr = array('wy_version'=>$config_arr['to_version'], 'wy_release'=>$config_arr['to_release']);
					//版本文件地址
	...
	public function checkfile() {
		if(!empty($_GET['do'])) {
			...
			//读取wycms接口
			$current_version = wy_base::load_config('version');
			$wycms_md5 = @file_get_contents($this->_upgrademd5.$current_version['wy_release'].'_'.CHARSET.".php");
			$wycms_md5_arr = json_decode($wycms_md5, 1);
			//计算数组差集
			$diff = array_diff($wycms_md5_arr, $this->md5_arr);

			//丢失文件列表
			$lostfile = array();
			foreach($wycms_md5_arr as $k=>$v) {
			...
			//未知文件列表
			$unknowfile = array_diff(array_keys($this->md5_arr), array_keys($wycms_md5_arr));
	...
	private function _wy_readdir($path='') {
		$dir_arr = explode('/', dirname($path));
		if(is_dir($path)) {
			$handler = opendir($path);
			while(($filename = @readdir($handler)) !== false) {
				if(substr($filename, 0, 1) != ".") {
					$this->_wy_readdir($path.'/'.$filename);
				}
			}
			closedir($handler);
		} else {
