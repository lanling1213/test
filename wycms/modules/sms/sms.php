...
class sms extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->log_db = wy_base::load_model('sms_report_model');
		...
		//初始化smsapi
		wy_base::load_app_class('smsapi', '', 0);
	...
	public function sms_sent() {
		...
		if(empty($this->smsapi->userid)) {
			showmessage(L('need_band'), 'index.php?m=sms&c=sms&a=sms_setting&menuid='.$_GET[menuid].'&wy_ssud='.$_SESSION['wy_ssud']);
		}
		//检查短信余额
