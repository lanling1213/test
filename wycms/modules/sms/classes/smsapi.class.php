...
class smsapi {
	...
	public function __construct($userid = '', $productid = '', $sms_key = '') {
		$this->smsapi_url = 'http://sms.wycms.cn/api.php?';
		$this->userid = $userid;
	...
	public function get_buyurl($productid = 0) {
		return 'http://sms.wycms.cn/index.php?m=sms_service&c=center&a=buy&sms_pid='.$this->productid.'&productid='.$productid;
	}
	...
	public function send_sms($mobile='', $content='', $send_time='', $charset='gbk',$id_code = '',$tplid = '',$return_code = 0) {
		...
		if($this->statuscode==0) {
			$barr = explode(':',$arr[1]);
			if($barr[0]=='KEY') {
				return '短信已提交，请等待审批！审批时间为：9:00-18:00。 法定假日不审批！如需帮助，请联系wycms.cn官网！';
			}
		}
		//end
		if($return_code) {
	 ...
	 * @param $timeout 超时时间
	 */
	public function wy_file_get_contents($timeout=30) {
