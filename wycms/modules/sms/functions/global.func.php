...
function get_smsnotice($type = '') {
	...
	$urls = 'http://sms.wycms.com/api.php?op=smsnotice&url='.$url."&type=".$type;
	$content = wy_file_get_contents($urls,5);
	if($content) {
	...
	$urls = 'http://sms.wycms.cn/api.php?op=smsnotice&url='.$url."&type=".$type;
	$content = wy_file_get_contents($urls,3);
	if($content) {
		$content = json_decode($content,true);
		if($content['status']==1) {
			return strtolower(CHARSET)=='gbk' ?iconv('utf-8','gbk',$content['msg']) : $content['msg'];
...
function sendsms($mobile, $send_txt, $tplid = 1, $id_code = '', $siteid=1) {
	...
	wy_base::load_app_class('smsapi', 'sms', 0); //引入smsapi类
