...
class manage extends admin {
	...
	function __construct() {
		...
		$this->db= wy_base::load_model('attachment_model');
		$this->attachment = wy_base::load_sys_class('attachment');
	...
	public function pulic_dirmode_del() {
		...
		$tmpdir = WYCMS_PATH.$tmpdirs[0].'/';
		if($tmpdir!=$this->upload_path) {
			showmessage(L('illegal_operation'));
		}
		$file = WYCMS_PATH.$dir.DIRECTORY_SEPARATOR.$filename;
	...
	public function delete() {
		$aid = $_GET['aid'];
		$attachment_index = wy_base::load_model('attachment_index_model');
