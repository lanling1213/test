<?php
	...
	function file_icon($file,$type = 'png') {
		...
		if(in_array($ext,$ext_arr)) return 'skins/images/ext/'.$ext.'.'.$type;
		else return 'skins/images/ext/blank.'.$type;
	}
	...
	function file_list($dirpath,$currentdir) {
		...
		$list['local'] = str_replace(array(WY_PATH, DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('',DIRECTORY_SEPARATOR), $filepath);
		return $list;
	}
	...
	function initupload($module, $catid,$args, $userid, $groupid = '8', $isadmin = '0',$userid_flash='0'){
		...
		$sess_id = SYS_TIME;
		$admin_url = wy_base::load_config('system','admin_url');
		$upload_path = empty($admin_url) ? APP_PATH : 'http://'.$admin_url.'/';
		$swf_auth_key = md5(wy_base::load_config('system','auth_key').$sess_id);
		$init =  'var swfu = \'\';
