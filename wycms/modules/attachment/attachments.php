...
class attachments {
	...
	function __construct() {
		...
		$this->upload_url = wy_base::load_config('system','upload_url');
		$this->upload_path = wy_base::load_config('system','upload_path');		
	...
	public function upload() {
		...
		wy_base::load_sys_class('attachment','',0);
	...
	public function swfupload(){
		...
		if(isset($_POST['dosubmit'])){
			if( $_POST['swf_auth_key'] != md5(wy_base::load_config('system','auth_key').$_POST['SWFUPLOADSESSID']) || ($_POST['isadmin']==0 && !$grouplist[$_POST['groupid']]['allowattachment'])) exit();
			wy_base::load_sys_class('attachment','',0);
	...
	public function crop_upload() {
		if (isset($GLOBALS["HTTP_RAW_POST_DATA"])) {
			...
			if (isset($_GET['file']) && !empty($_GET['file'])) {
				...
				if (strpos($_GET['file'], wy_base::load_config('system', 'upload_url'))!==false) {
					$file = $_GET['file'];
			...
			echo wy_base::load_config('system', 'upload_url').$filepath.$new_file;
			exit;
		}
	}
	...
	public function swfdelete() {
		$attachment = wy_base::load_sys_class('attachment');
	...
	public function album_load() {
		...
		$where = $uploadtime= '';
		$this->att_db= wy_base::load_model('attachment_model');
	...
	public function album_dir() {
		...
		$local = str_replace(array(WY_PATH, WYCMS_PATH ,DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('','',DIRECTORY_SEPARATOR), $filepath);
