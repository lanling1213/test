...
class index {
    ...
    public function listdwy() {
        ...
        if (!isset($CATEGORYS[$catid]))
            exit(L('parameter_error'));
        $CAT = $CATEGORYS[$catid];
        $template = $CAT['child'] == 1 ? $WAP_SETTING['category_template'] : $WAP_SETTING['list_template'];
        include template('wap', $template);
    }
    //展示列表页
	...
	public function show() {
		...
		$contentpage = wy_base::load_app_class('contentpage','content');
		...
		if($pictureurls) {
			...
			if($PIC_POS !== false) {
				$this->url = wy_base::load_app_class('wap_url', 'wap');
	...
	function comment() {
		...
		if($_POST['dosumbit']) {
			...
			wy_base::load_app_func('global','comment');
			$username = $this->wap['sitename'].L('wycms_friends');
			...
			echo L('wap_guestbook').'<br/><a href="'.show_url($catid,$contentid,$typeid).'">'.L('wap_goback').'</a><script language=javascript>setTimeout("redirect(\''.HTTP_REFERER.'\');",0);</script>';
		}
	}
	...
	function comment_list() {
		...
		$comment_setting_db = wy_base::load_model('comment_setting_model');
	...
	function big_image() {
        ...
        $WAP_SETTING = string2array($WAP['setting']);
        if (isset($_POST)) {
            $catid = isset($_POST['catid']) && intval($_POST['catid']) > 0 ? intval($_POST['catid']) : 0;
            $page = intval($_POST['page']);
            $siteid = max($this->siteid, 1);
            $MODEL = getcache('model', 'commons');
            if ($page < 1)
            ...
            if ($catid) {
                $CATEGORYS = getcache('category_content_' . $siteid, 'commons');
                $CAT = $CATEGORYS[$catid];
                if (empty($CATEGORYS) || empty($CAT))
                    exit(json_encode(array()));
                $modelid = $CAT['modelid'];
                if ($CAT['child']) {
                    $where = "catid in(" . $CAT['arrchildid'] . ") and status=99";
                } else {
            ...
            $pagesize = isset($_POST['num']) && intval($_POST['num']) <= intval($WAP_SETTING['listnum']) ? intval($_POST['num']) : intval($WAP_SETTING['listnum']);
            $offset = ($page - 1) * $pagesize;
            $list = $this->db->select($where, 'id,catid,title,description,url,thumb,inputtime', $offset . ',' . $pagesize, 'listorder desc,id desc');
    ...
    function ajaxAskList()
    {
        ...
        if ($_GET['ajax']==1) {
            ...
            $list = $this->db->listinfo('',$order = 'inputtime DESC,questionid DESC',$page, 2);
             $this->dbreply = wy_base::load_model('ask_reply_model');

             foreach($list as &$n)
             {
                 $res =$this->dbreply->select(array('questionid'=>$n['questionid']));
                 $n[inputtime]=date('y-m-d',$n[inputtime]);
                 $n['reply_content'] = $res[0]['content'];
             }
            
            if ($list) {
                exit(json_encode(array('status' => 1, 'data' => $list)));
            } else {
                exit(json_encode(array('status' => 0, 'data' => '')));
            }
        } else {
            exit(json_encode(''));
        }
