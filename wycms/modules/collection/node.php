...
class node extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = wy_base::load_model('collection_node_model');
	...
	public function node_import() {
		if (isset($_POST['dosubmit'])) {
			...
			if (wy_base::load_config('system', 'charset') == 'gbk') {
	...
	public function export() {
		...
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
		    ...
		    header("Content-Disposition: attachment; filename=wy_collection_".$nodeid.'.txt');
	...
	public function del() {
		if (isset($_POST['dosubmit'])) {
			...
			$content_db = wy_base::load_model('collection_content_model');
	...
	public function public_test() {
		$nodeid = isset($_GET['nodeid']) ? intval($_GET['nodeid']) : showmessage(L('illegal_parameters'), HTTP_REFERER);
		wy_base::load_app_class('collection', '', 0);
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
	...
	public function public_name() {
		$name = isset($_GET['name']) && trim($_GET['name']) ? (wy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['name'])) : trim($_GET['name'])) : exit('0');
	...
	public function col_url_list() {
		...
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
			...
			if ($total_page > 0) {
				...
				$history_db = wy_base::load_model('collection_history_model');
				$content_db = wy_base::load_model('collection_content_model');
				$total = count($url);
				$re = 0;
	...
	public function col_content() {
		...
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
			...
			if ($total_page > $page) {
				showmessage(L('collectioning').($i+($page-1)*2).'/'.$total.'<script type="text/javascript">location.href="?m=collection&c=node&a=col_content&page='.($page+1).'&nodeid='.$nodeid.'&total='.$total.'&wy_ssud='.$_SESSION['wy_ssud'].'"</script>', '?m=collection&c=node&a=col_content&page='.($page+1).'&nodeid='.$nodeid.'&total='.$total);
			} else {
				$this->db->update(array('lastdate'=>SYS_TIME), array('nodeid'=>$nodeid));
	...
	public function import() {
		...
		$program_db = wy_base::load_model('collection_program_model');
	...
	public function content_del() {
		...
		if (is_array($id)) {
			$collection_content_db = wy_base::load_model('collection_content_model');
			$history_db = wy_base::load_model('collection_history_model');
			$del_array = $id;
			$ids = implode('\',\'', $id);
			if ($history) {
	...
	public function import_content() {
		...
		$program_db = wy_base::load_model('collection_program_model');
		$collection_content_db = wy_base::load_model('collection_content_model');
		$content_db = wy_base::load_model('content_model');
		//更新附件状态
		$attach_status = false;
		if(wy_base::load_config('system','attachment_stat')) {
			$attachment_db = wy_base::load_model('attachment_model');
			$att_index_db = wy_base::load_model('attachment_index_model');
			$attach_status = true;
		}
		$order = $node['coll_order'] == 1 ? 'id desc' : '';
		$str = L('operation_success');
		$url = '?m=collection&c=node&a=publist&nodeid='.$nodeid.'&status=2&wy_ssud='.$_SESSION['wy_ssud'];
		if ($type == 'all') {
		...
		if ($type == 'all' && $total_page > $page) {
			$str = L('are_imported_the_import_process').(($page-1)*20+$i).'/'.$total.'<script type="text/javascript">location.href="?m=collection&c=node&a=import_content&nodeid='.$nodeid.'&programid='.$programid.'&type=all&page='.($page+1).'&total='.$total.'&wy_ssud='.$_SESSION['wy_ssud'].'"</script>';
			$url = '';
		}
		showmessage($str, $url);
	}
