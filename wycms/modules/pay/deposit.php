...
wy_base::load_app_class('foreground','member');
...
class deposit extends foreground {
	...
	function __construct() {
		...
		parent::__construct();
		$this->pay_db = wy_base::load_model('pay_payment_model');
		$this->account_db = wy_base::load_model('pay_account_model');
		$this->_username = param::get_cookie('_username');
		$this->_userid = intval(param::get_cookie('_userid'));
		$this->handle = wy_base::load_app_class('pay_deposit');
	}

	public function init() {
		wy_base::load_app_class('pay_factory','',0);
		$where = '';
