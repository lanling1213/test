...
class payment extends admin {
	...
	function __construct() {
		...
		parent::__construct();
		$this->db = wy_base::load_model('pay_payment_model');
		$this->account_db = wy_base::load_model('pay_account_model');
		$this->member_db = wy_base::load_model('member_model');
		$this->modules_path = WY_PATH.'modules'.DIRECTORY_SEPARATOR.'pay';
		wy_base::load_app_class('pay_method','','0');
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($userinfo) {	
				...
				if($_POST['pay_unit']) {
					...
					$receipts = wy_base::load_app_class('receipts');
					...
					$spend = wy_base::load_app_class('spend');
					$func = $_POST['pay_type'] == '1' ? 'amount' :'point';
