...
class comment_tag {
	...
	public function __construct() {
		$this->comment_db = wy_base::load_model('comment_model');
		$this->comment_setting_db = wy_base::load_model('comment_setting_model');
		$this->comment_data_db = wy_base::load_model('comment_data_model');
		$this->comment_table_db = wy_base::load_model('comment_table_model');
	}
	
	/**
	 * 
	 * WY标签数据数量计算函数
	 * @param array $data WY标签中的配置参数传入
	 */
	 ...
	 * WY标签，可视化显示参数配置。
	 */
	public function wy_tag() {
		$sites = wy_base::load_app_class('sites','admin');
		$sitelist = $sites->wy_tag_list();
		return array(
