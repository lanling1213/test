...
class index {
	...
	public function init() {
		...
		$siteid =& $this->siteid;
		$username = param::get_cookie('_username',L('wycms_friends'));
	...
	public function post() {
		$comment = wy_base::load_app_class('comment');
		$id = isset($_GET['id']) && intval($_GET['id']) ? intval($_GET['id']) : '';
		$SITE = siteinfo($this->siteid);
		$username = param::get_cookie('_username',$SITE['name'].L('wycms_friends'));
		$userid = param::get_cookie('_userid');
		$comment_setting_db = wy_base::load_model('comment_setting_model');
		$setting = $comment_setting_db->get_one(array('siteid'=>$this->siteid));
		if (!empty($setting)) {
	...
	public function ajax() {
		...
		$wy_tag = wy_base::load_app_class('comment_tag');
		$comment = array();
		if ($comment = $wy_tag->get_comment(array('commentid'=>$commentid))) {
			$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
			$offset = ($page-1)*$num;
			$data = array('commentid'=>$commentid, 'site'=>$siteid, 'limit'=>$offset.','.$num, 'direction'=>$direction);
			$comment['data'] = $wy_tag->lists($data);
			wy_base::load_sys_class('format', '', 0);
	...
	protected function _show_msg($msg, $url = '', $status = 0) {
		...
		switch ($this->format) {
			case 'json':
				$msg = wy_base::load_config('system', 'charset') == 'gbk' ? iconv('gbk', 'utf-8', $msg) : $msg;
