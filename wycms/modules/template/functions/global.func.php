...
function tag_md5($file) {
	...
	preg_match_all("/\{wy:(\w+)\s+([^}]+)\}/i", stripslashes($data),$matches);
	$arr = array();
 ...
 * @param $dir 目录名
 */
function replace_wy_tag($filepath, $old_tag, $new_tag, $style, $dir) {
...
function creat_template_bak($filepath, $style, $dir) {
	...
	$template_bak_db = wy_base::load_model('template_bak_model');
...
function creat_form($id, $data, $value = '', $op = '') {
	...
	if($data['ajax']['name']) {
		if($data['ajax']['m']) {
			$url = '$.get(\'?m=content&c=push&a=public_ajax_get\', {html: this.value, id:\''.$data['ajax']['id'].'\', action: \''.$data['ajax']['action'].'\', module: \''.$data['ajax']['m'].'\', wy_ssud: \''.$_SESSION['wy_ssud'].'\'}, function(data) {$(\'#'.$id.'_td\').html(data)});';
		} else {
			$url = '$.get(\'?m=template&c=file&a=public_ajax_get\', { html: this.value, id:\''.$data['ajax']['id'].'\', action: \''.$data['ajax']['action'].'\', op: \''.$op.'\', style: \'default\', wy_ssud: \''.$_SESSION['wy_ssud'].'\'}, function(data) {$(\'#'.$id.'_td\').html(data)});';
		}
	}
...
function visualization($html, $style = '', $dir = '', $file = '') {
	$change = "<link href=\"".CSS_PATH."dialog.css\" rel=\"stylesheet\" type=\"text/css\" />
		...
		var wy_ssud = '".$_SESSION['wy_ssud']."';
		$(function(){
		$('a').attr('href', 'javascript:void(0)').attr('target', '');
		$('.admin_piao_edit').click(function(){
		var url = '?m=template&c=file&a=edit_wy_tag';
		if($(this).parent('.admin_piao').attr('wy_action') == 'block') url = '?m=block&c=block_admin&a=add';
		window.top.art.dialog({title:'".L('wy_tag','' ,'template')."',id:'edit',iframe:url+'&style=$style&dir=$dir&file=$file&'+$(this).parent('.admin_piao').attr('data'),width:'700',height:'500'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});})
		$('.admin_block').click(function(){
			window.top.art.dialog({title:'".L('wy_tag','' ,'template')."',id:'edit',iframe:'?m=block&c=block_admin&a=block_update&id='+$(this).attr('blockid'),width:'700',height:'500'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
		});
	})</script><div id=\"WY__contentHeight\" style=\"display:none\">80</div>";
