...
class member_verify extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = wy_base::load_model('member_verify_model');
		$this->_init_wysso();
	}
	...
	function modelinfo() {
		...
		//模型字段名称
		$this->member_field_db = wy_base::load_model('sitemodel_field_model');
	...
	function pass() {
		if (isset($_POST['userid'])) {
			...
			//wysso注册失败的用户状态直接置为审核期间wysso已注册该会员
	...
	public function checkemail_ajax() {
		...
		if(isset($_GET['wyssouid']) && isset($status['uid'])) {
			if ($status['uid'] == intval($_GET['wyssouid'])) {
				exit('1');
			}
		}
