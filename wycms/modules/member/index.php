...
class index extends foreground {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($member_setting[mobile_checktype]=='1'){
				...
				if($mobile_verify=='') showmessage('请提供正确的手机验证码！', HTTP_REFERER);
 				$sms_report_db = wy_base::load_model('sms_report_model');
			...
			if($member_setting['enablemailcheck']) {	//是否需要邮件验证
				...
				$this->verify_db = wy_base::load_model('member_verify_model');
			...
			if(wy_base::load_config('system', 'wysso')) {
				...
				if($status > 0) {
					$userinfo['wyssouid'] = $status;
					//传入wysso为明文密码，加密后存入wycms
					...
					if($userid > 0) {
						...
						if($userinfo['groupid'] == 7) {
							...
							$wycms_auth = sys_auth($userid."\t".$userinfo['password'], 'ENCODE', get_auth_key('login'));
							
							param::set_cookie('auth', $wycms_auth, $cookietime);
					...
					if($member_setting['enablemailcheck']) {
						...
						param::set_cookie('_reguseruid', $userinfo['wyssouid'], $cookietime);
						showmessage(L('operation_success'), 'index.php?m=member&c=index&a=register&t=2');
					} else {
						//如果不需要邮箱认证、直接登录其他应用
						$synloginstr = $this->client->ps_member_synlogin($userinfo['wyssouid']);
						showmessage(L('operation_success').$synloginstr, 'index.php?m=member&c=index&a=init');
					}
					
				}
			} else {
				showmessage(L('enable_register').L('enable_wysso'), 'index.php?m=member&c=index&a=login');
			}
	...
	public function account_manage() {
		...
		foreach($model_info as $k=>$v) {
			...
			if($v['formtype'] == 'image') {
				$member_modelinfo[$v['name']] = "<a href='$member_modelinfo_arr[$k]' target='_blank'><img src='$member_modelinfo_arr[$k]' height='40' widht='40' onerror=\"this.src='$wysso_api_url/skins/images/member/nophoto.gif'\"></a>";
				...
				if(is_array($tmp)) {
					foreach ($tmp as $tv) {
						$member_modelinfo[$v['name']] .= " <a href='$tv[url]' target='_blank'><img src='$tv[url]' height='40' widht='40' onerror=\"this.src='$wysso_api_url/skins/images/member/nophoto.gif'\"></a>";
					}
					unset($tmp);
				}
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(wy_base::load_config('system', 'wysso')) {
				...
				if(isset($memberinfo['uid'])) {
					//查询帐号
					$r = $this->db->get_one(array('wyssouid'=>$memberinfo['uid']));
					if(!$r) {
						//插入会员详细信息，会员不存在 插入会员
						$info = array(
									'wyssouid'=>$memberinfo['uid'],
					...
					$password = $r['password'];
					$synloginstr = $this->client->ps_member_synlogin($r['wyssouid']);
 				} else {
					if($status == -1) {	//用户不存在
				...
				//密码错误剩余重试次数
				$this->times_db = wy_base::load_model('times_model');
			...
			$wycms_auth = sys_auth($userid."\t".$password, 'ENCODE', get_auth_key('login'));
	...
	public function favorite() {
		$this->favorite_db = wy_base::load_model('favorite_model');
		$memberinfo = $this->memberinfo;
	...
	public function friend() {
		$memberinfo = $this->memberinfo;
		$this->friend_db = wy_base::load_model('friend_model');
		if(isset($_GET['friendid'])) {
			...
			foreach($friendids as $k=>$v) {
				...
				$friendlist[$k]['avatar'] = $this->client->ps_getavatar($v['wyssouid']);
	...
	public function change_credit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$status = $this->client->ps_changecredit($memberinfo['wyssouid'], $from, $toappid, $to, $fromvalue);
			if($status == 1) {
				if($from == 1) {
			...
			$credit_list = wy_base::load_config('credit');
	...
	public function mini() {
		...
		$snda_enable = wy_base::load_config('system', 'snda_enable');
	...
	public function public_checkemail_ajax() {
		...
		if($status == -5) {	//禁止注册
			...
			if(isset($_GET['wyssouid'])) {	//修改用户传入wyssouid
				$status = $this->client->ps_get_member_info($email, 3);
				if($status) {
					$status = unserialize($status);	//接口返回序列化，进行判断
					if (isset($status['uid']) && $status['uid'] == intval($_GET['wyssouid'])) {
						exit('1');
					} else {
						exit('0');
					}
	...
	public function public_sina_login() {
		define('WB_AKEY', wy_base::load_config('system', 'sina_akey'));
		define('WB_SKEY', wy_base::load_config('system', 'sina_skey'));
		define('WEB_CALLBACK', APP_PATH.'index.php?m=member&c=index&a=public_sina_login&callback=1');
		wy_base::load_app_class('saetv2.ex', '' ,0);
		$this->_session_start();
	...
	public function public_snda_login() {
		define('SNDA_AKEY', wy_base::load_config('system', 'snda_akey'));
		define('SNDA_SKEY', wy_base::load_config('system', 'snda_skey'));
		define('SNDA_CALLBACK', urlencode(APP_PATH.'index.php?m=member&c=index&a=public_snda_login&callback=1'));
		
		wy_base::load_app_class('OauthSDK', '' ,0);
		$this->_session_start();		
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
	...
	public function public_qq_loginnew(){
                $appid = wy_base::load_config('system', 'qq_appid');
                $appkey = wy_base::load_config('system', 'qq_appkey');
                $callback = wy_base::load_config('system', 'qq_callback');
                wy_base::load_app_class('qqapi','',0);
	...
	public function public_qq_login() {
		define('QQ_AKEY', wy_base::load_config('system', 'qq_akey'));
		define('QQ_SKEY', wy_base::load_config('system', 'qq_skey'));
		wy_base::load_app_class('qqoauth', '' ,0);
		$this->_session_start();
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(is_array($code) && is_numeric($code[0]) && date('y-m-d h', SYS_TIME) == date('y-m-d h', $code[1])) {
				...
				if(wy_base::load_config('system', 'wysso')) {
					//初始化wysso
					$this->_init_wysso();
					$this->client->ps_member_edit('', $email, '', $password, $memberinfo['wyssouid'], $memberinfo['encrypt']);
				}
				$email = $memberinfo['email'];
				//获取站点名称
				$sitelist = getcache('sitelist', 'commons');		
	...
	/**
	*通过手机修改密码
	*方式：用户发送HHPWD afei985#821008 至 1065788 ，WYCMS进行转发到网站运营者指定的回调地址，在回调地址程序进行密码修改等操作,处理成功时给用户发条短信确认。
	*wycms 以POST方式传递相关数据到回调程序中
	...
	*/
	...
	public function public_changepwd_bymobile(){
		...
		//判断是否WYCMS请求的接口
		wy_base::load_app_func('global','sms');
		wy_base::load_app_class('smsapi', 'sms', 0);
		...
		//处理提交申请，以手机号为准
			...
			if($mobile){
				...
				if($r && $r['id_code']==$mobile_verify) {
					...
					$rs = $this->db->get_one(array('userid'=>$userid),'wyssouid');
					if(wy_base::load_config('system', 'wysso')) {
						//初始化wysso
						$this->_init_wysso();
						$this->client->ps_member_edit('', '', '', $password, $rs['wyssouid'], $encrypt);
					}
