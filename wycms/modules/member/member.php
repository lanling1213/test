...
class member extends admin {
	...
	function search() {
		...
		foreach($memberlist as $k=>$v) {
			$memberlist[$k]['avatar'] = get_memberavatar($v['wyssouid']);
		}
		$pages = $this->db->pages;
	...
	function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			if(!empty($_POST['delavatar'])) {
				$this->client->ps_deleteavatar($userinfo['wyssouid']);
			}

			$status = $this->client->ps_member_edit($info['username'], $info['email'], '', $info['password'], $userinfo['wyssouid'], $userinfo['encrypt']);
  			if($status >= 0) { 
				unset($info['userid']);
				unset($info['username']);
			...
			$memberinfo['avatar'] = get_memberavatar($memberinfo['wyssouid'], '', 90);
	...
	function delete() {
		...
		$wysso_userinfo = $this->db->listinfo($where);
		$wyssouidarr = array();
		if(is_array($wysso_userinfo)) {
			foreach($wysso_userinfo as $v) {
				if(!empty($v['wyssouid'])) {
					$wyssouidarr[] = $v['wyssouid'];
				}
			}
		}
		//查询用户信息
		...
		//delete wysso member first
		if(!empty($wyssouidarr)) {
			$status = $this->client->ps_delete_member($wyssouidarr, 1);
			if($status > 0) {
	...
	function memberinfo() {
		...
		foreach($model_fieldinfo as $k=>$v) {
			if($v['formtype'] == 'image') {
				$member_modelinfo[$k] = "<a href='.$member_modelinfo[$k].' target='_blank'><img src='.$member_modelinfo[$k].' height='40' widht='40' onerror=\"this.src='$wysso_api_url/skins/images/member/nophoto.gif'\"></a>";
			} elseif($v['formtype'] == 'images') {
				$tmp = string2array($member_modelinfo[$k]);
				$member_modelinfo[$k] = '';
				if(is_array($tmp)) {
					foreach ($tmp as $tv) {
						$member_modelinfo[$k] .= " <a href='$tv[url]' target='_blank'><img src='$tv[url]' height='40' widht='40' onerror=\"this.src='$wysso_api_url/skins/images/member/nophoto.gif'\"></a>";
					}
					unset($tmp);
				}
