...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$("#email").formValidator({onshow:"<?php echo L('input').L('email')?>",onfocus:"<?php echo L('email').L('format_incorrect')?>",oncorrect:"<?php echo L('email').L('format_right')?>"}).regexValidator({regexp:"email",datatype:"enum",onerror:"<?php echo L('email').L('format_incorrect')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"m=member&c=member&a=public_checkemail_ajax&wyssouid=<?php echo $memberinfo['wyssouid']?>",
		datatype : "html",
		async:'false',
...
<!--
	function changemodel(modelid) {
		redirect('?m=member&c=member&a=edit&userid=<?php echo $memberinfo[userid]?>&modelid='+modelid+'&wy_ssud=<?php echo $_SESSION['wy_ssud']?>');
	}
//-->
</script>
</html>
