...
class form {
	...
	public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1,$alowuploadexts = '',$height = 200,$disabled_page = 0, $allowuploadnum = '10') {
		...
		$ext_str .= "</div>";
		if(is_ie()) $ext_str .= "<div style='display:none'><OBJECT id='WY_Capture' classid='clsid:021E8C6F-52D4-42F2-9B36-BCFBAD3A0DE4'><PARAM NAME='_Version' VALUE='0'><PARAM NAME='_ExtentX' VALUE='0'><PARAM NAME='_ExtentY' VALUE='0'><PARAM NAME='_StockProps' VALUE='0'></OBJECT></div>";
		$str .= $ext_str;
		return $str;
	}
	...
	public static function select_category($file = '',$catid = 0, $str = '', $default_option = '', $modelid = 0, $type = -1, $onlysub = 0,$siteid = 0,$is_push = 0) {
		...
		if($is_push=='1'){
			$priv = wy_base::load_model('category_priv_model');
	...
	public static function select_template($style, $module, $id = '', $str = '', $pre = '') {
		$tpl_root = wy_base::load_config('system', 'tpl_root');
		$templatedir = WY_PATH.$tpl_root.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR;
		$confing_path = WY_PATH.$tpl_root.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'config.php';
	 ...
	 */
	public static function checkcode($id = 'alidatecode', $code_len = 4, $font_size = 20, $width = 130, $height = 50, $font = '', $font_color = '', $background = '') {
        $geetest_open = wy_base::load_config('system', 'geetest_open');
        if (1 == $geetest_open) { 
            $product = is_numeric($code_len) ?'float' :$code_len; // 参数范围：float、embed、popup
            $siteurl = SITE_PROTOCOL.SITE_URL;
            $html = <<<EOT
				<script src="http://static.geetest.com/static/tools/gt.js"></script>
				<script type="text/javascript">
				    $.ajax({
				        // 获取id，challenge，success（是否启用failback）
				        url: "{$url}/api.php?op=alidatecode&rand="+Math.round(Math.random()*100),
				        type: "get",
...
EOT;
            ...
            return "<img id='$id' style='cursor:pointer' onclick='this.src=this.src+\"&\"+Math.random()' src='".SITE_PROTOCOL.SITE_URL.WEB_PATH."api.php?op=alidatecode&code_len=$code_len&font_size=$font_size&width=$width&height=$height&font_color=".urlencode($font_color)."&background=".urlencode($background)."'>";
        }
	}
     ...
     */
    public static function verifycode($name='code')
    {
        $geetest_open = wy_base::load_config('system', 'geetest_open');
        if (1 == $geetest_open) { 
            $checkcode = wy_base::load_sys_class('checkcode_geetest');
            $status = $checkcode->verify_code();
        } else {
            $checkcode = wy_base::load_sys_class('checkcode');
            $status = $checkcode->verify_code($name);
        }
