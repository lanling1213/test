...
function pluginlang($language = 'no_language',$pars = array(), $plugin = '') {
	...
	if(!$PLUGIN_LANG && defined('PLUGIN_ID')) {
		if(file_exists(WY_PATH.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.PLUGIN_ID.DIRECTORY_SEPARATOR.'languages'.DIRECTORY_SEPARATOR.PLUGIN_ID.'.lang.php')) require WY_PATH.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.PLUGIN_ID.DIRECTORY_SEPARATOR.'languages'.DIRECTORY_SEPARATOR.PLUGIN_ID.'.lang.php';
	}	
	if(!empty($plugin)) {
		require WY_PATH.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'languages'.DIRECTORY_SEPARATOR.$plugin.'.lang.php';
	}
...
function plugin_stat($appid = '') {
	if(wy_base::load_config('system','plugin_debug')) return 2;
	...
	$url = 'http://open.wycms.cn/api.php?op=appstatus&'.$data.'&appid='.$appid;
...
function plugin_install_stat($appid){
	if(wy_base::load_config('system','plugin_debug')) return false;
	$appid = intval($appid);
	if($appid == 0 ) return false;
	$url = 'http://open.wycms.cn/api.php?op=appstatus&isinstall=1';
...
function app_validity_check($appid) {
	...
	$appid = intval($appid);
	$url = $header = '';
	$url = 'http://open.wycms.cn/api.php?op=appstatus';
	$headers = get_headers($url.'&appid='.$appid,1);
	$status = $headers['pc_appstatus'];
	return $status;
}
