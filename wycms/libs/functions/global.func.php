...
function sys_auth($string, $operation = 'ENCODE', $key = '', $expiry = 0) {
	$ckey_length = 4;
	$key = md5($key != '' ? $key : wy_base::load_config('system', 'auth_key'));
...
function L($language = 'no_language',$pars = array(), $modules = '') {
	...
	if(defined('IN_ADMIN')) {
		...
		$lang = wy_base::load_config('system','lang');
	}
	if(!$LANG) {
		require_once WY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system.lang.php';
		if(defined('IN_ADMIN')) require_once WY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system_menu.lang.php';
		if(file_exists(WY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php')) require_once WY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php';
	}
	if(!empty($modules)) {
		$modules = explode(',',$modules);
		foreach($modules AS $m) {
			if(!isset($LANG_MODULES[$m])) require_once WY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.$m.'.lang.php';
		}
	}
...
function template($module = 'content', $template = 'index', $style = '') {
	...
	$template_cache = wy_base::load_sys_class('template_cache');
	$compiledtplfile = WYCMS_PATH.'data'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
	if(file_exists(WY_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
		if(!file_exists($compiledtplfile) || (@filemtime(WY_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > @filemtime($compiledtplfile))) {
			$template_cache->template_compile($module, $template, $style);
		}
	} else {
		$compiledtplfile = WYCMS_PATH.'data'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
		if(!file_exists($compiledtplfile) || (file_exists(WY_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(WY_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile))) {
			$template_cache->template_compile($module, $template, 'default');
		} elseif (!file_exists(WY_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
 ...
 * showmessage('登录成功', array('默认跳转地址'=>'http://www.wycms.cn'));
 ...
 * @param $name 缓存名称
 * @param $data 缓存数据
 * @param $filepath 数据路径（模块名称） data/cache_$filepath/
...
function setcache($name, $data, $filepath='', $type='file', $config='', $timeout=0) {
	...
	wy_base::load_sys_class('cache_factory','',0);
	if($config) {
		$cacheconfig = wy_base::load_config('cache');
...
function getcacheinfo($name, $filepath='', $type='file', $config='') {
	if(!preg_match("/^[a-zA-Z0-9_-]+$/", $name)) return false;
	if($filepath!="" && !preg_match("/^[a-zA-Z0-9_-]+$/", $filepath)) return false;
	wy_base::load_sys_class('cache_factory');
	if($config) {
		$cacheconfig = wy_base::load_config('cache');
		$cache = cache_factory::get_instance($cacheconfig)->get_cache($config);
	} else {
 ...
 * @return 分页
 */
function wy_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 4) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			...
			$multipage .= ' <a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="link"><</a>';
			if($curr_page==1) {
				$multipage .= ' <a class="link1" href="#">1</a>';
		...
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				...
				$multipage .= ' <a class="link1" href="#">'.$i.'</a>';
			}
		}
               
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="link">></a>';
			} else {
				$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="link">></a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <a class="link1" href="#">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page, $array).'" class="link">></a>';
		} else {
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="link">></a>';
		}
	}
...
function show_ad($siteid, $id) {
	...
	$p = wy_base::load_model('poster_model');
...
/**
 * 获取用户头像，建议传入wyssouid
 * @param $uid 默认为wyssouid
 * @param $is_userid $uid是否为v9 userid，如果为真，执行sql查询此用户的wyssouid
 * @param $size 头像大小 有四种[30x30 45x45 90x90 180x180] 默认30
 */
function get_memberavatar($uid, $is_userid='', $size='30') {
	if($is_userid) {
		$db = wy_base::load_model('member_model');
		$memberinfo = $db->get_one(array('userid'=>$uid));
		if(isset($memberinfo['wyssouid'])) {
			$uid = $memberinfo['wyssouid'];
		} else {
			return false;
		}
	}

	wy_base::load_app_class('client', 'member', 0);
	...
	$client = new client($wysso_api_url, $wysso_auth_key);
...
function thumb($imgurl, $width = 100, $height = 100 ,$autocut = 1, $smallpic = 'nopic.gif') {
	global $image;
	$upload_url = wy_base::load_config('system','upload_url');
	$upload_path = wy_base::load_config('system','upload_path');
	...
	if(!is_object($image)) {
		wy_base::load_sys_class('image','','0');
		$image = new image(1,0);
	}
...
function go($catid,$id, $allurl = 0) {
	...
	if(!$modelid) return '';
	$db = wy_base::load_model('content_model');
...
function upload_key($args) {
	$pc_auth_key = md5(wy_base::load_config('system','auth_key').$_SERVER['HTTP_USER_AGENT']);
...
function get_auth_key($prefix,$suffix="") {
	if($prefix=='login'){
		$pc_auth_key = md5(wy_base::load_config('system','auth_key').ip());
	}else if($prefix=='email'){
		$pc_auth_key = md5(wy_base::load_config('system','auth_key'));
	}else{
		$pc_auth_key = md5(wy_base::load_config('system','auth_key').$suffix);
	}
 ...
 * @param string $font 字体名称 字体请放于wycms\libs\data\font下
...
function get_pc_version($type='') {
	$version = wy_base::load_config('version');
	if($type==1) {
...
function runhook($method) {
	...
	if(!empty($hook_appid)) {
		foreach($hook_appid as $appid => $p) {
			$pluginfilepath = WY_PATH.'plugin'.DIRECTORY_SEPARATOR.$p.DIRECTORY_SEPARATOR.'hook.class.php';
			$getpclass = TRUE;
...
function p_template($plugin = 'content', $template = 'index',$style='default') {
	if(!$style) $style = 'default';
	$template_cache = wy_base::load_sys_class('template_cache');
	$compiledtplfile = WYCMS_PATH.'data'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.$template.'.php';

	if(!file_exists($compiledtplfile) || (file_exists(WY_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(WY_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile))) {
		$template_cache->template_compile('plugin/'.$plugin, $template, 'default');
	} elseif (!file_exists(WY_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html')) {
...
function get_vid($contentid = 0, $catid = 0, $isspecial = 0) {
	...
	if (!$isspecial) {
		...
		$video_content = wy_base::load_model('video_content_model');
		$r = $video_content->get_one(array('contentid'=>$contentid, 'modelid'=>$modelid), 'videoid', '`listorder` ASC');
		$video_store =wy_base::load_model('video_store_model');
		return $video_store->get_one(array('videoid'=>$r['videoid']));
	} else {
		$special_content = wy_base::load_model('special_content_model');
		$contentid = intval($contentid);
		$video_store =wy_base::load_model('video_store_model');
