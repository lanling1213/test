<?php
/**
 *  base.php WYCMS框架入口文件
 *
 * @copyright			(C) 2005-2010 WYCMS
 * @license				http://www.wycms.cn/license/
 * @lastmodify			2010-6-7
 */
define('IN_WYCMS', true);

//WYCMS框架路径
define('WY_PATH', dirname(__FILE__).DIRECTORY_SEPARATOR);

if(!defined('WYCMS_PATH')) define('WYCMS_PATH', WY_PATH.'..'.DIRECTORY_SEPARATOR);

//缓存文件夹地址
define('CACHE_PATH', WYCMS_PATH.'data'.DIRECTORY_SEPARATOR);
//主机协议
...
//加载公用函数库
wy_base::load_sys_func('global');
wy_base::load_sys_func('extention');
wy_base::auto_load_func();

wy_base::load_config('system','errorlog') ? set_error_handler('my_error_handler') : error_reporting(E_ERROR | E_WARNING | E_PARSE);
//设置本地时差
function_exists('date_default_timezone_set') && date_default_timezone_set(wy_base::load_config('system','timezone'));

define('CHARSET' ,wy_base::load_config('system','charset'));
//输出页面字符集
header('Content-type: text/html; charset='.CHARSET);

define('SYS_TIME', time());
//定义网站根路径
define('WEB_PATH',wy_base::load_config('system','web_path'));
//js 路径
define('JS_PATH',wy_base::load_config('system','js_path'));
//css 路径
define('CSS_PATH',wy_base::load_config('system','css_path'));
//img 路径
define('IMG_PATH',wy_base::load_config('system','img_path'));
//动态程序路径
define('APP_PATH',wy_base::load_config('system','app_path'));

//应用静态文件路径
define('PLUGIN_STATICS_PATH',WEB_PATH.'skins/plugin/');

if(wy_base::load_config('system','gzip') && function_exists('ob_gzhandler')) {
...
class wy_base {
	...
	private static function _load_class($classname, $path = '', $initialize = 1) {
		...
		if (file_exists(WY_PATH.$path.DIRECTORY_SEPARATOR.$classname.'.class.php')) {
			include WY_PATH.$path.DIRECTORY_SEPARATOR.$classname.'.class.php';
			$name = $classname;
			if ($my_path = self::my_path(WY_PATH.$path.DIRECTORY_SEPARATOR.$classname.'.class.php')) {
				include $my_path;
				$name = 'MY_'.$classname;
			}
			if ($initialize) {
	...
	public static function load_plugin_class($classname, $identification = '' ,$initialize = 1) {
		...
		return wy_base::load_sys_class($classname, 'plugin'.DIRECTORY_SEPARATOR.$identification.DIRECTORY_SEPARATOR.'classes', $initialize);
	}
	...
	public static function load_plugin_func($func,$identification) {
		...
		if (file_exists(WY_PATH.$path)) {
			include WY_PATH.$path;
		} else {
			$funcs[$key] = false;
			return false;
		}
	...
	private static function _auto_load_func($path = '') {
		...
		$auto_funcs = glob(WY_PATH.DIRECTORY_SEPARATOR.$path);
