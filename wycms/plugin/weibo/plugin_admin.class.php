<?php
	...
	class plugin_admin {
		...
		function __construct($pluginid) {
			...
			$this->op = wy_base::load_app_class('plugin_op');
			$this->auth_db = wy_base::load_plugin_model('weibo_auth_info_model');
		}
		
		public function oauth() {
			wy_base::load_plugin_class('weibooauth','',0);
			...
			if(!isset($_REQUEST['oauth_verifier']) || $_REQUEST['oauth_verifier'] == '') {
				if($this->auth_db->get_one(array('source'=>'sina'))) {
					$txt = '授权成功，<a href="?m=admin&c=plugin&a=config&pluginid='.$this->pluginid.'&module=removeauth&wy_ssud='.$_SESSION['wy_ssud'].'">解除绑定</a>';
				} else {
				...
				$txt = '授权成功，'.$me['name'].'<a href="?m=admin&c=plugin&a=config&pluginid='.$this->pluginid.'&module=removeauth&wy_ssud='.$_SESSION['wy_ssud'].'">解除绑定</a>';
			}
