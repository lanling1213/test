...
final class template_cache {
	...
	public function template_compile($module, $template, $style = 'default') {
		...
		$tplfile = $_tpl = WY_PATH.'templates/'.$style.'/'.$module.'/'.$template.'.html';
		if ($style != 'default' && ! file_exists ( $tplfile )) {
			$tplfile = WY_PATH.'templates/default/'.$module.'/'.$template.'.html';
		}
		...
		$filepath = WYCMS_PATH.'data/caches_template/'.$module.'/';
	    if(!is_dir($filepath)) {
	...
	public function template_compile_admin($module, $template) {
		...
		$tplfile = $_tpl = WY_PATH.'templates/admin/'.$module.'/'.$template.'.html';
		$content = @file_get_contents ( $tplfile );
		$filepath = WYCMS_PATH.'data/caches_template/admintpl/'.$module.'/';
	    if(!is_dir($filepath)) {
			mkdir($filepath, 0777, true);
	    }
