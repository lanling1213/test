...
class param {
	...
	public function __construct() {
		...
		$this->route_config = wy_base::load_config('route', SITE_URL) ? wy_base::load_config('route', SITE_URL) : wy_base::load_config('route', 'default');
	...
	public static function set_cookie($var, $value = '', $time = 0) {
		...
		$var = wy_base::load_config('system','cookie_pre').$var;
		$_COOKIE[$var] = $value;
		if (is_array($value)) {
			foreach($value as $k=>$v) {
				setcookie($var.'['.$k.']', sys_auth($v, 'ENCODE'), $time, wy_base::load_config('system','cookie_path'), wy_base::load_config('system','cookie_domain'), $s);
			}
		} else {
			setcookie($var, sys_auth($value, 'ENCODE'), $time, wy_base::load_config('system','cookie_path'), wy_base::load_config('system','cookie_domain'), $s);
		}
	}
