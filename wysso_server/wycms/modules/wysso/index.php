...
wy_base::load_app_class('wysso', 'wysso', 0);
wy_base::load_app_class('messagequeue', 'admin' , 0);
wy_base::load_app_func('global', 'admin');

class index extends wysso {
	
	private $username, $config;
	
	public function __construct() {
		parent::__construct();
		$this->config = wy_base::load_config('system');
		/*判断应用字符集和wysso字符集是否相同，如果不相同，转换用户名为wysso所用字符集*/
		...
		if ($this->username && CHARSET != $this->applist[$this->appid]['charset']) {
			if($this->applist[$this->appid]['charset'] == 'utf-8') {	//判断应用字符集是否为utf-8编码
				//应用字符集如果是utf-8，并且用户名是utf-8编码，转换用户名为wysso字符集，如果为英文，is_utf8返回false，不进行转换
	...
	public function register() {
		...
		if ($this->config['ucuse']) {
			wy_base::load_config('uc_config');
			require_once WYCMS_PATH.'api/uc_client/client.php';
	...
	public function login() {
		...
		if($userinfo) {
			...
			if ($this->config['ucuse']) {
				if ($uid < 0) {	//用户不存在或者密码错误
					...
					//当前使用只在uc中存在，在WYSSO中是不存在的。需要进行注册。
					wy_base::load_sys_class('uc_model', 'model', 0);
		...
		//如果开启wycms_2008_sp4兼容模式，根据sp4规则验证密码，如果不成功再根据wysso规则验证密码
	...
	public function uploadavatar() {
		...
		//创建图片存储文件夹
		$avatarfile = wy_base::load_config('system', 'upload_path').'avatar/';
