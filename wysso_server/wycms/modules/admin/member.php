		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->db->get_one(array('username'=>$username))) {
				...
				if ($this->config['ucuse']) {
					...
					include WYCMS_PATH.'api/uc_client/client.php';
				...
				if ($uid = $this->db->insert(array('username'=>$username, 'password'=>$password, 'random'=>$random, 'email'=>$email, 'regdate'=>$regdate, 'lastdate'=>SYS_TIME, 'type'=>'app', 'regip'=>ip(), 'appname'=>'wysso', 'ucuserid'=>$ucuserid), 1)) {
					/*插入消息队列*/
