 ...
 */
	...
	$system = wy_base::load_config('system');
	define('APPID', $system['wysso_appid']);
	$ps_api_url = $system['wysso_api_url'];	//接口地址
	$ps_auth_key = $system['wysso_auth_key'];	//加密密钥
	$ps_version = $system['wysso_version'];
	...
	if ($action == 'member_add') {
		...
		$userinfo = array();
		$userinfo['wyssouid'] = isset($arr['uid']) ? intval($arr['uid']) : exit('0');
	...
	if ($action == 'member_edit') {
		...
		$status = $db->update($userinfo, array('wyssouid'=> $arr['uid']));
		if($status) {
			exit('1');
		} else {
			exit('0');
	...
	if ($action == 'synlogin') {
		...
		$arr['uid'] = intval($arr['uid']);
		$wyssouid = $arr['uid'];
		$userinfo = $db->get_one(array('wyssouid'=>$wyssouid));
		...
		wy_base::load_sys_class('param', '', 0);
