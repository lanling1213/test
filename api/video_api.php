...
//新增，跳转完善资料  -- 增加人：wgq
$wy_ssud = htmlspecialchars($_GET['wy_ssud']);
if(!empty($_GET['do_complete']) && !empty($_GET['uid']) && !empty($_GET['snid'])){
	...
	showmessage("请完善注册信息资料！",APP_PATH.'index.php?m=video&c=video&a=complete_info&uid='.floor(floatval($_GET['uid'])).'&snid='.$sndaid.'&wy_ssud='.$wy_ssud);
}

if(!empty($_GET['skey']) && !empty($_GET['sn'])){
	header("Location: ".APP_PATH.'index.php?m=video&c=video&a=set_video_setting&skey='.$_GET['skey'].'&sn='.$_GET['sn'].'&wy_ssud='.$wy_ssud); 
}
...
$xxtea = wy_base::load_app_class('xxtea', 'video');
$token = $_POST['token'];
...
if (!preg_match('/([^a-z_]+)/i',$action) && file_exists(WYCMS_PATH.'api/video_api/'.$action.'.php')) {
	include WYCMS_PATH.'api/video_api/'.$action.'.php';
} else {
